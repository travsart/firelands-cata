...
Firelands::Crypto::AES::AES(bool encrypting) : _ctx(EVP_CIPHER_CTX_new()), _encrypting(encrypting)
{
...
Firelands::Crypto::AES::~AES()
{
    EVP_CIPHER_CTX_free(_ctx);
}

void Firelands::Crypto::AES::Init(Key const& key)
{
    int status = EVP_CipherInit_ex(_ctx, nullptr, nullptr, key.data(), nullptr, -1);
    ASSERT(status);
}

bool Firelands::Crypto::AES::Process(IV const& iv, uint8* data, std::size_t length, Tag& tag)
{
