...
enum RPEventLines2
{
    RP2_LINE_ARTHAS1 = 10,  // Everyone looks ready. Remember, these people are all infected with the plague and will die soon. We must purge Stratholme to protect the remainder of Lordaeron from the
                            // Scourge. Let's go.
    ...
    RP2_LINE_ARTHAS5 = 14 // Mal'Ganis will send out some of his Scourge minions to interfere with us. Those of you with the strongest steel and magic shall go forth and destroy them. I will lead the
                          // rest of my forces in purging Stratholme of the infected.
};
...
enum RPEventLines4
{
    ...
    RP4_LINE_ARTHAS13 =
        34 // At last some good luck. Market Row has not caught fire yet. Mal'Ganis is supposed to be in Crusaders' Square, which is just ahead. Tell me when you're ready to move forward.
};
...
enum RPEventLines5
{
    ...
    RP5_LINE_MALGANIS12 = 10, // Your journey has just begun, young prince. Gather your forces and meet me in the arctic land of Northrend. It is there that we shall settle the score between us. It is
                              // there that your true destiny will unfold.
...
G3D::Vector3 const ChromieSplinePos[chromiePathSize] = {{2320.632f, 1507.193f, 152.5081f}, {2319.823f, 1506.605f, 152.5081f}, {2306.770f, 1496.780f, 128.3620f}};
...
static std::unordered_map<uint32, SnapbackInfo> const ArthasSnapbackPositions = {{JUST_STARTED, {REACT_PASSIVE, false, &ArthasPositions[RP1_ARTHAS_INITIAL]}},
    {CRATES_IN_PROGRESS, {REACT_PASSIVE, false, &ArthasPositions[RP1_ARTHAS_INITIAL]}}, {CRATES_DONE, {REACT_PASSIVE, false, &ArthasPositions[RP1_ARTHAS_INITIAL]}},
    {UTHER_TALK, {REACT_PASSIVE, false, &ArthasPositions[RP1_ARTHAS_INITIAL]}}, {PURGE_PENDING, {REACT_PASSIVE, true, &ArthasPositions[ARTHAS_PURGE_PENDING_POS]}},
    {PURGE_STARTING, {REACT_PASSIVE, false, &ArthasPositions[ARTHAS_PURGE_PENDING_POS]}}, {WAVES_IN_PROGRESS, {REACT_AGGRESSIVE, false, &ArthasPositions[ARTHAS_WAVES_POS]}},
    {WAVES_DONE, {REACT_DEFENSIVE, false, &ArthasPositions[ARTHAS_WAVES_POS]}}, {TOWN_HALL_PENDING, {REACT_DEFENSIVE, true, &ArthasPositions[ARTHAS_TOWN_HALL_POS]}},
    {TOWN_HALL, {REACT_DEFENSIVE, false, &ArthasPositions[ARTHAS_TOWN_HALL_POS]}}, {TOWN_HALL_COMPLETE, {REACT_PASSIVE, true, &ArthasPositions[ARTHAS_TOWN_HALL_END_POS]}},
    {GAUNTLET_TRANSITION, {REACT_PASSIVE, false, &ArthasPositions[ARTHAS_TOWN_HALL_END_POS]}}, {GAUNTLET_PENDING, {REACT_PASSIVE, true, &ArthasPositions[ARTHAS_GAUNTLET_POS]}},
    {GAUNTLET_IN_PROGRESS, {REACT_DEFENSIVE, false, &ArthasPositions[ARTHAS_GAUNTLET_POS]}}, {GAUNTLET_COMPLETE, {REACT_PASSIVE, true, &ArthasPositions[ARTHAS_GAUNTLET_END_POS]}},
    {MALGANIS_IN_PROGRESS, {REACT_DEFENSIVE, false, &ArthasPositions[ARTHAS_GAUNTLET_END_POS]}}, {COMPLETE, {REACT_PASSIVE, false, &ArthasPositions[ARTHAS_FINAL_POS]}}};
...
class npc_arthas_stratholme : public CreatureScript
{
  public:
    ...
    struct npc_arthas_stratholmeAI : public ScriptedAI
    {
        ...
        {
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: advancing to 0x%X", newState);
            if (!_progressRP)
            {
                LOG_WARN("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: advancing to instance state 0x%X, but RP is paused. Overriding!", newState);
                _progressRP = true;
            }
            ...
            if (itr != ArthasSnapbackPositions.end())
            {
                ...
                LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::AdvanceToState: has snapback for this state, distance = %f", target.SnapbackPosition->GetExactDist(me));
        ...
        {
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: RP in progress? '%s'", _progressRP ? "YES" : "NO");
            if (_progressRP)
            {
                _progressRP = false;
                ...
                if (!_resumeMovement.Empty())
                    LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: spline chain motion paused");
                else
                    LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustEngagedWith: entered combat without pathing, pausing RP regardless");
            }
        ...
        {
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::EnterEvadeMode: why = %u ", why);
        ...
        void JustReachedHome() override
        {
            ...
            {
                LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustReachedHome: Resuming motion");
                me->GetMotionMaster()->ResumeSplineChain(_resumeMovement);
                _resumeMovement.Clear();
            }
            else
                LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::JustReachedHome: Back at leash pos, resuming RP");
        ...
        bool GossipSelect(Player* player, uint32 /*sender*/, uint32 listId) override
        {
            ...
            LOG_TRACE("scripts.cos", "npc_arthas_stratholmeAI::GossipSelect: '%s' selects action '%u' on '%s'", player->GetGUID().ToString().c_str(), action, me->GetGUID().ToString().c_str());
