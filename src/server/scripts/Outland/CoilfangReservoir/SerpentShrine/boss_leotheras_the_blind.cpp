            ...
            else
            {
                ...
                if (ChaosBlast_Timer <= diff)
                {
                    ...
                    if (me->IsWithinDist(me->GetVictim(), 30))
                    {
                        ...
                        int damage = 100;
                        DoCast(me->GetVictim(), SPELL_CHAOS_BLAST, CastSpellExtraArgs(damage).SetOriginalCaster(me->GetGUID()));
                    }
                ...
                if (InnerDemons_Timer <= diff)
                {
                    ...
                    for (auto itr = TargetList.begin(), end = TargetList.end(); itr != end; ++itr)
                    {
                        ...
                        {
                            Creature* demon =
                                me->SummonCreature(INNER_DEMON_ID, (*itr)->GetPositionX() + 10, (*itr)->GetPositionY() + 10, (*itr)->GetPositionZ(), 0, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 5000);
                            if (demon)
                            {
