...
uint32 const RepairSpells[4] = {SPELL_SEAT_1, SPELL_SEAT_2, SPELL_SEAT_3, SPELL_SEAT_5};

Position const VehicleRelocation[] = {
    {0.0f, 0.0f, 0.0f}, {2792.070f, 2596.320f, 364.3136f}, // WP_MKII_P1_IDLE
    ...
    struct boss_mimironAI : public BossAI
    {
        ...
        void JustDied(Unit* /*killer*/) override
        {
            ...
            me->GetMotionMaster()->MoveJump(
                me->GetPositionX() + (10.f * std::cos(me->GetOrientation())), me->GetPositionY() + (10.f * std::sin(me->GetOrientation())), me->GetPositionZ(), me->GetOrientation(), 10.f, 5.f);
            ...
            while (uint32 eventId = events.ExecuteEvent())
            {
                ...
                case EVENT_OUTTRO_2:
                    ...
                    if (_fireFighter)
                    {
                        if (Creature* computer = instance->GetCreature(DATA_COMPUTER))
                            computer->AI()->DoAction(DO_DEACTIVATE_COMPUTER);
                        me->SummonGameObject(RAID_MODE(GO_CACHE_OF_INNOVATION_FIREFIGHTER, GO_CACHE_OF_INNOVATION_FIREFIGHTER_HERO), 2744.040f, 2569.352f, 364.3135f, 3.124123f,
                            QuaternionData(0.f, 0.f, 0.9999619f, 0.008734641f), 604800);
                    }
                    else
                        me->SummonGameObject(
                            RAID_MODE(GO_CACHE_OF_INNOVATION, GO_CACHE_OF_INNOVATION_HERO), 2744.040f, 2569.352f, 364.3135f, 3.124123f, QuaternionData(0.f, 0.f, 0.9999619f, 0.008734641f), 604800);
  ...
  public:
    ...
    class spell_mimiron_napalm_shell_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            targets.remove_if(Firelands::AllWorldObjectsInRange(GetCaster(), 15.0f));
