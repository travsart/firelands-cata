                ...
                switch (action)
                {
                    ...
                    case ACTION_ROTFACE_COMBAT:
                    {
                        ...
                        if (Creature* rotface = ObjectAccessor::GetCreature(*me, instance->GetGuidData(DATA_ROTFACE)))
                        {
                            ...
                            if (list.size() > 4)
                            {
                                list.sort(Firelands::ObjectDistanceOrderPred(rotface));
                                do
                                {
                                    list.pop_back();
                                } while (list.size() > 4);
    ...
    public:
        ...
        class spell_putricide_ooze_eruption_searcher_SpellScript : public SpellScript
        {
            void HandleDummy(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    GetHitUnit()->RemoveAurasDueToSpell(adhesiveId, GetCaster()->GetGUID(), 0, AuraRemoveMode::ByEnemySpell);
    ...
    public:
        ...
        class spell_putricide_unbound_plague_SpellScript : public SpellScript
        {
            ...
            void FilterTargets(std::list<WorldObject*>& targets)
            {
                ...
                targets.remove_if(Firelands::UnitAuraCheck(true, sSpellMgr->GetSpellIdForDifficulty(SPELL_UNBOUND_PLAGUE, GetCaster())));
