...
{
  public:
    ValithriaDelayedCastEvent(Creature* trigger, uint32 spellId, ObjectGuid originalCaster, uint32 despawnTime)
        : _trigger(trigger), _originalCaster(originalCaster), _spellId(spellId), _despawnTime(despawnTime)
    {
    }
...
class ValithriaDespawner : public BasicEvent
{
  public:
    ...
    {
        Firelands::CreatureWorker<ValithriaDespawner> worker(_creature, *this);
...
class npc_green_dragon_combat_trigger : public CreatureScript
{
  public:
    ...
    struct npc_green_dragon_combat_triggerAI : public BossAI
    {
        ...
        {
        } // JustEnteredCombat is already doing all of this hook's default actions so no need to execute it twice
        ...
        void UpdateAI(uint32 diff) override
        {
            ...
            while (uint32 eventId = _events.ExecuteEvent())
            {
                ...
                case EVENT_CHECK_PLAYER:
                {
                    ...
                    Firelands::AnyPlayerInObjectRangeCheck check(me, 5.0f);
                    Firelands::PlayerSearcher<Firelands::AnyPlayerInObjectRangeCheck> searcher(me, player, check);
...
class spell_dreamwalker_summoner : public SpellScriptLoader
{
  public:
    ...
    class spell_dreamwalker_summoner_SpellScript : public SpellScript
    {
        ...
        {
            targets.remove_if(Firelands::UnitAuraCheck(true, SPELL_RECENTLY_SPAWNED));
            if (targets.empty())
                return;
    ...
    class spell_dreamwalker_summon_suppresser_AuraScript : public AuraScript
    {
        ...
        void HandleSummon(Unit* caster)
        {
            ...
            for (uint8 i = 0; i < suppresserNumber; ++i)
                caster->CastSpell(Firelands::Containers::SelectRandomContainerElement(summoners), SPELL_SUMMON_SUPPRESSER, true);
        }
