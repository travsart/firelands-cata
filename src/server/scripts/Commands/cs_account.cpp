...
public:
    ...
    static bool HandleAccount2FASetupCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto token = Firelands::StringTo<uint32>(args);
        ...
        static std::unordered_map<uint32, Firelands::Crypto::TOTP::Secret> suggestions;
        auto pair = suggestions.emplace(std::piecewise_construct, std::make_tuple(accountId), std::make_tuple(Firelands::Crypto::TOTP::RECOMMENDED_SECRET_LENGTH)); // std::vector 1-argument std::size_t constructor invokes resize

        if (pair.second) // no suggestion yet, generate random secret
            Firelands::Crypto::GetRandomBytes(pair.first->second);

        if (!pair.second && token) // suggestion already existed and token specified - validate
        {
            if (Firelands::Crypto::TOTP::ValidateToken(pair.first->second, *token))
            {
                if (masterKey)
                    Firelands::Crypto::AEEncryptWithRandomIV<Firelands::Crypto::AES>(pair.first->second, *masterKey);
        ...
        handler->SendErrorMessage(LANG_2FA_SECRET_SUGGESTION, Firelands::Encoding::Base32::Encode(pair.first->second).c_str());
        return false;
    }
    ...
    static bool HandleAccount2FARemoveCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 const accountId = handler->GetSession()->GetAccountId();
        Firelands::Crypto::TOTP::Secret secret;
        ...
        if (token)
        {
            if (masterKey)
            {
                bool success = Firelands::Crypto::AEDecrypt<Firelands::Crypto::AES>(secret, *masterKey);
                if (!success)
                {
            ...
            if (Firelands::Crypto::TOTP::ValidateToken(secret, *token))
            {
                auto* stmt = LoginDatabase.GetPreparedStatement(LOGIN_UPD_ACCOUNT_TOTP_SECRET);
                stmt->SetData(0);
                stmt->SetData(1, accountId);
    ...
    static bool HandleAccountAddonCommand(ChatHandler* handler, char const* args)
    {
        ...
        auto expansion = Firelands::StringTo<uint8>(exp); //get int anyway (0 if error)
    ...
    static bool HandleAccountSet2FACommand(ChatHandler* handler, char const* args)
    {
        ...
        Optional<std::vector<uint8>> decoded = Firelands::Encoding::Base32::Decode(secret);
        if (!decoded)
        {
        ...
        if (128 < (decoded->size() + Firelands::Crypto::AES::IV_SIZE_BYTES + Firelands::Crypto::AES::TAG_SIZE_BYTES))
        {
        ...
        if (masterKey)
            Firelands::Crypto::AEEncryptWithRandomIV<Firelands::Crypto::AES>(*decoded, *masterKey);
    ...
    static bool HandleAccountSetGmLevelCommand(ChatHandler* handler, char const* args)
    {
        ...
        gm = (isAccountNameGiven) ? Firelands::StringTo<int32>(arg2).value_or(0) : Firelands::StringTo<int32>(arg1).value_or(0);
        if (gm > SEC_CONSOLE)
        {
        ...
        int32 gmRealmID = (isAccountNameGiven) ? Firelands::StringTo<int32>(arg3).value_or(0) : Firelands::StringTo<int32>(arg2).value_or(0);
