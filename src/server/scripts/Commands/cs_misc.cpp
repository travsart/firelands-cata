...
public:
    ...
    static bool HandleSkirmishCommand(ChatHandler* handler, std::vector<std::string_view> args)
    {
        ...
        auto arenaTokens = Firelands::Tokenize(arenasStr, ',', false);
        ...
        BattlegroundTypeId randomizedArenaBgTypeId = Firelands::Containers::SelectRandomContainerElement(allowedArenas);
    ...
    static bool HandleGPSCommand(ChatHandler* handler, Optional<PlayerIdentifier> target)
    {
        ...
        CellCoord const cellCoord = Firelands::ComputeCellCoord(object->GetPositionX(), object->GetPositionY());
        Cell cell(cellCoord);
    ...
    {
        Firelands::ChatCommands::SendCommandHelpFor(*handler, cmd);

        if (cmd.empty())
        {
            Firelands::ChatCommands::SendCommandHelpFor(*handler, "help");
        }
    ...
    static bool HandleMuteCommand(ChatHandler* handler, Optional<PlayerIdentifier> player, std::string notSpeakTime, Tail muteReason)
    {
        ...
        if (Firelands::StringTo<int32>(notSpeakTime).value_or(0) < 0)
        {
            handler->SendErrorMessage(LANG_BAD_VALUE);
            return false;
        }
        ...
        if (muteDuration <= 0)
        {
            muteDuration = Firelands::StringTo<int32>(notSpeakTime).value_or(0);
        }
    ...
    static bool HandleMuteInfoHelper(ChatHandler* handler, uint32 accountId, char const* accountName)
    {
        ...
        do
        {
            ...
            handler->PSendSysMessage(LANG_COMMAND_MUTEHISTORY_OUTPUT, Firelands::Time::TimeToHumanReadable(Seconds(fields[0].Get<uint32>())), fields[1].Get<uint32>(), fields[2].Get<std::string>(), fields[3].Get<std::string>());
