...
public:
    ...
    static bool HandleSendItemsCommand(ChatHandler* handler, Optional<PlayerIdentifier> target, QuotedString subject, QuotedString text, Tail items)
    {
        ...
        for (auto const& itemString : Firelands::Tokenize(items, ' ', true))
        {
            auto itemTokens = Firelands::Tokenize(itemString, ':', false);
            ...
            switch (itemTokens.size())
            {
                case 1:
                    ...
                    break;
                case 2:
                    itemCount = *Firelands::StringTo<uint32>(itemTokens.at(1));
                    break;
                default:
                    handler->SendSysMessage(Firelands::StringFormat("> Incorrect item list format for '{}'", itemString));
                    continue;
            }

            uint32 itemID = *Firelands::StringTo<uint32>(itemTokens.at(0));
