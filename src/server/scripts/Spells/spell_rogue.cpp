                ...
                if (Unit* target = GetHitUnit())
                {
                    ...
                    for (uint8 slot = 0; slot < MAX_ENCHANTMENT_SLOT; ++slot)
                    {
                        ...
                        for (uint8 s = 0; s < 3; ++s)
                        {
                            ...
                            if (!spellInfo)
                            {
                                LOG_ERROR("spells", "Player::CastItemCombatSpell Enchant %i, player (Name: %s, GUID: %u) cast unknown spell %i", enchant->ID, player->GetName().c_str(), player->GetGUID().GetCounter(), enchant->EffectArg[s]);
                                continue;
                            }
...
{
    bool Validate(SpellInfo const* /*spellInfo*/) override { return ValidateSpellInfo({SPELL_ROGUE_STEALTH}); }
    ...
    void Register() override { AfterEffectApply.Register(&spell_rog_vanish::ApplyStealth, EFFECT_1, SPELL_AURA_MOD_STEALTH, AURA_EFFECT_HANDLE_REAL_OR_REAPPLY_MASK); }
};
