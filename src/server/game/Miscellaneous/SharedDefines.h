...
#define RACEMASK_ALL_PLAYABLE                                                                                                                                                                          \
    ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_ORC - 1)) | (1 << (RACE_DWARF - 1)) | (1 << (RACE_NIGHTELF - 1)) | (1 << (RACE_UNDEAD_PLAYER - 1)) | (1 << (RACE_TAUREN - 1)) | (1 << (RACE_GNOME - 1)) |   \
        (1 << (RACE_TROLL - 1)) | (1 << (RACE_BLOODELF - 1)) | (1 << (RACE_DRAENEI - 1)) | (1 << (RACE_GOBLIN - 1)) | (1 << (RACE_WORGEN - 1)))

#define RACEMASK_ALLIANCE ((1 << (RACE_HUMAN - 1)) | (1 << (RACE_DWARF - 1)) | (1 << (RACE_NIGHTELF - 1)) | (1 << (RACE_GNOME - 1)) | (1 << (RACE_DRAENEI - 1)) | (1 << (RACE_WORGEN - 1)))
...
#define CLASSMASK_ALL_PLAYABLE                                                                                                                                                                         \
    ((1 << (CLASS_WARRIOR - 1)) | (1 << (CLASS_PALADIN - 1)) | (1 << (CLASS_HUNTER - 1)) | (1 << (CLASS_ROGUE - 1)) | (1 << (CLASS_PRIEST - 1)) | (1 << (CLASS_SHAMAN - 1)) |                          \
        (1 << (CLASS_MAGE - 1)) | (1 << (CLASS_WARLOCK - 1)) | (1 << (CLASS_DRUID - 1)) | (1 << (CLASS_DEATH_KNIGHT - 1)))
...
enum SpellAttr0 : uint32
{
    ...
    SPELL_ATTR0_ON_NEXT_SWING_NO_DAMAGE = 0x00000004,                       // On Next Swing (No Damage) DESCRIPTION Both "on next swing" attributes
                                                                            // have identical handling in server & client
    ...
    SPELL_ATTR0_DO_NOT_DISPLAY_SPELLBOOK_AURA_ICON_COMBAT_LOG = 0x00000080, // Do Not Display (Spellbook, Aura Icon, Combat Log) (client only) DESCRIPTION Not visible in spellbook or
                                                                            // aura bar
    ...
    SPELL_ATTR0_ALLOW_CAST_WHILE_DEAD = 0x00800000,            // Allow Cast While Dead DESCRIPTION Spells without this flag cannot be cast
                                                               // by dead units in non-triggered contexts
    SPELL_ATTR0_ALLOW_WHILE_MOUNTED = 0x01000000,              // Allow While Mounted
    SPELL_ATTR0_COOLDOWN_ON_EVENT = 0x02000000,                // Cooldown On Event DESCRIPTION Spell is unusable while already active, and
                                                               // cooldown does not begin until the effects have worn off
    ...
    SPELL_ATTR0_NO_IMMUNITIES = 0x20000000,                    // No Immunities DESCRIPTION Allows spell to pierce invulnerability, unless the
                                                               // invulnerability spell also has this attribute
...
enum SpellAttr3
{
    ...
    SPELL_ATTR3_TREAT_AS_PERIODIC = 0x02000000,         // 25 Makes the spell appear as periodic in client combat logs - used by spells
                                                        // that trigger another spell on each tick
...
enum SpellAttr4
{
    SPELL_ATTR4_IGNORE_RESISTANCES = 0x00000001,              //  0 spells with this attribute will completely ignore the target's resistance
                                                              //  (these spells can't be resisted)
    ...
    SPELL_ATTR4_CAN_CAST_WHILE_CASTING = 0x00000080,          //  7 Can be cast while another cast is in progress - see
                                                              //  CanCastWhileCasting(SpellRec const*,CGUnit_C *,int &)
    ...
    SPELL_ATTR4_CANT_TRIGGER_ITEM_SPELLS = 0x00800000,        // 23 spells with this flag should not trigger item spells / enchants
                                                              // (mostly in conjunction with SPELL_ATTR0_CANCELS_AUTO_ATTACK_COMBAT)
...
enum SpellAttr5
{
    ...
    SPELL_ATTR5_DONT_TURN_DURING_CAST = 0x00080000,            // 19 Blocks caster's turning when casting (client does not automatically turn
                                                               // caster's model to face UNIT_FIELD_TARGET)
    ...
    SPELL_ATTR5_SKIP_CHECKCAST_LOS_CHECK = 0x04000000,         // 26 aoe related - Boulder, Cannon, Corpse Explosion, Fire Nova, Flames, Frost Bomb, Living Bomb, Seed of
                                                               // Corruption, Starfall, Thunder Clap, Volley
...
enum SpellAttr6
{
    ...
    SPELL_ATTR6_USE_SPELL_CAST_EVENT = 0x00000040,         //  6 Auras with this attribute trigger SPELL_CAST combat log event instead of
                                                           //  SPELL_AURA_START (clientside attribute)
    ...
    SPELL_ATTR6_CAST_BY_CHARMER = 0x00040000,              // 18 client won't allow to cast these spells when unit is not possessed && charmer
                                                           // of caster will be original caster
...
enum SpellAttr7
{
    SPELL_ATTR7_ALLOW_SPELL_REFLECTION = 0x00000001,           //  0 Shaman's new spells (Call of the ...), Feign Death.
    SPELL_ATTR7_IGNORE_DURATION_MODS = 0x00000002,             //  1 Duration is not affected by duration modifiers
    SPELL_ATTR7_DISABLE_AURA_WHILE_DEAD = 0x00000004,          //  2 Paladin's auras and 65607 only.
    ...
    SPELL_ATTR7_HIDDEN_IN_SPELLBOOK_WHEN_LEARNED = 0x00010000, // 16 After learning these spells become hidden in spellbook (but
                                                               // are visible when not learned for low level characters)
...
enum SpellCustomErrors
{
    ...
    SPELL_CUSTOM_ERROR_WYRMREST_VANQUISHER = 20,              // You may only call for the aid of a Wyrmrest Vanquisher in Wyrmrest Temple, The
                                                              // Dragon Wastes, Galakrond's Rest or The Wicked Coil.
    ...
    SPELL_CUSTOM_ERROR_REQUIRES_GAMMOTH_MAGNATAUR = 24,       // You may only use the Pouch of Crushed Bloodspore on Gammothra or other
                                                              // magnataur in the Bloodspore Plains and Gammoth.
    ...
    SPELL_CUSTOM_ERROR_ALREADY_CARRYING_CRUSADER = 43,        // You are already carrying a captured Argent Crusader. You must return to the Argent Vanguard infirmary and drop off
                                                              // your passenger before you may pick up another.
    ...
    SPELL_CUSTOM_ERROR_DESTROYED_KFC_OIL_PLATFORM = 91,       // You have already destroyed the KTC Oil Platform.
...
{                                           // (C) used in caster aura state     (T) used in target aura state
    ...
    AURA_STATE_HUNTER_PARRY = 7,            // C   |
                                            // AURA_STATE_UNKNOWN7                   = 7,            //  c  | creature cheap shot / focused
                                            // bursts spells AURA_STATE_UNKNOWN8                   = 8,            //    t| test spells
...
#define IMMUNE_TO_MOVEMENT_IMPAIRMENT_AND_LOSS_CONTROL_MASK                                                                                                                                            \
    ((1 << MECHANIC_CHARM) | (1 << MECHANIC_DISORIENTED) | (1 << MECHANIC_FEAR) | (1 << MECHANIC_ROOT) | (1 << MECHANIC_SLEEP) | (1 << MECHANIC_SNARE) | (1 << MECHANIC_STUN) |                        \
        (1 << MECHANIC_FREEZE) | (1 << MECHANIC_SILENCE) | (1 << MECHANIC_DISARM) | (1 << MECHANIC_KNOCKOUT) | (1 << MECHANIC_POLYMORPH) | (1 << MECHANIC_BANISH) | (1 << MECHANIC_SHACKLE) |          \
        (1 << MECHANIC_TURN) | (1 << MECHANIC_HORROR) | (1 << MECHANIC_DAZE) | (1 << MECHANIC_SAPPED))
...
// TARGET_[OBJECT_TYPE]_[REFERENCE_TYPE(skipped for caster)]_[SELECTION_TYPE(skipped for default)]_[additional
// specifiers(friendly, BACK_LEFT, etc.]
enum Targets
{
...
enum TotemCategory
{
    ...
    FC_TRUESILVER_ROD = 9,
    FC_ARCANITE_ROD = 10,
    FC_MINING_PICK_OLD = 11,
    FC_PHILOSOPHERS_STONE = 12,
    FC_BLACKSMITH_HAMMER_OLD = 13,
    FC_ARCLIGHT_SPANNER = 14,
    FC_GYROMATIC_MA = 15,
    FC_MASTER_TOTEM = 21,
    FC_FEL_IRON_ROD = 41,
    FC_ADAMANTITE_ROD = 62,
    FC_ETERNIUM_ROD = 63,
    FC_HOLLOW_QUILL = 81,
    FC_RUNED_AZURITE_ROD = 101,
    FC_VIRTUOSO_INKING_SET = 121,
    FC_DRUMS = 141,
    FC_GNOMISH_ARMY_KNIFE = 161,
    FC_BLACKSMITH_HAMMER = 162,
    FC_MINING_PICK = 165,
    FC_SKINNING_KNIFE = 166,
    FC_HAMMER_PICK = 167,
    FC_BLADED_PICKAXE = 168,
    FC_FLINT_AND_TINDER = 169,
    FC_RUNED_COBALT_ROD = 189,
    FC_RUNED_TITANIUM_ROD = 190,
    FC_RUNED_ELEMENTIUM_ROD = 209,
    FC_HIGH_POWERED_BOLT_GUN = 210,
};
