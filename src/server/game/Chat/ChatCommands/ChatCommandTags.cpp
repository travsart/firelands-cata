...
using namespace Firelands::Impl::ChatCommands;

ChatCommandResult Firelands::ChatCommands::QuotedString::TryConsume(ChatHandler const* handler, std::string_view args)
{
    if (args.empty())
        return std::nullopt;
...
ChatCommandResult Firelands::ChatCommands::AccountIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
    ...
    Optional<uint32> id = Firelands::StringTo<uint32>(text, 10);
    if (!id)
...
ChatCommandResult Firelands::ChatCommands::PlayerIdentifier::TryConsume(ChatHandler const* handler, std::string_view args)
{
...
Firelands::ChatCommands::PlayerIdentifier::PlayerIdentifier(Player& player)
    : _name(player.GetName()), _guid(player.GetGUID()), _player(&player) {}

/*static*/ Optional<Firelands::ChatCommands::PlayerIdentifier> Firelands::ChatCommands::PlayerIdentifier::FromTarget(ChatHandler* handler)
{
...
/*static*/ Optional<Firelands::ChatCommands::PlayerIdentifier> Firelands::ChatCommands::PlayerIdentifier::FromSelf(ChatHandler* handler)
{
    if (Player* player = handler->GetPlayer())
        return { *player };
