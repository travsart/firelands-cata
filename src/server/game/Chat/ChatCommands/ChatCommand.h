...
namespace Firelands::Impl::ChatCommands
{
        ...
        static ChatCommandResult TryConsumeTo(Tuple& tuple, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (result1.HasErrorMessage() && result2.HasErrorMessage())
            {
                return Firelands::StringFormat("{} \"{}\"\n{} \"{}\"",
                    GetFirelandsString(handler, LANG_CMDPARSER_EITHER), result2.GetErrorMessage(),
                    GetFirelandsString(handler, LANG_CMDPARSER_OR), result1.GetErrorMessage());
            }
    ...
    template <typename T> struct HandlerToTuple { static_assert(Firelands::dependant_false_v<T>, "Invalid command handler signature"); };
    ...
    struct CommandPermissions
    {
        ...
        CommandPermissions(uint32 securityLevel, Firelands::ChatCommands::Console console) : RequiredLevel{ securityLevel }, AllowConsole{ console } {}
        uint32 RequiredLevel;
        Firelands::ChatCommands::Console AllowConsole;
    };

    class ChatCommandNode
    {
    friend struct FilteredCommandListIterator;
    using ChatCommandBuilder = Firelands::ChatCommands::ChatCommandBuilder;
    ...
    private:
        ...
        static void LoadCommandsIntoMap(ChatCommandNode* blank, std::map<std::string_view, Firelands::Impl::ChatCommands::ChatCommandNode, StringCompareLessI_T>& map, Firelands::ChatCommands::ChatCommandTable const& commands);
...
namespace Firelands::ChatCommands
{
    struct ChatCommandBuilder
    {
        friend class Firelands::Impl::ChatCommands::ChatCommandNode;

        struct InvokerEntry
        {
            template <typename T>
            InvokerEntry(T& handler, AcoreStrings help, uint32 securityLevel, Firelands::ChatCommands::Console allowConsole)
            ...
            Firelands::Impl::ChatCommands::CommandInvoker _invoker;
            AcoreStrings _help;
            Firelands::Impl::ChatCommands::CommandPermissions _permissions;
        ...
        template <typename TypedHandler>
        ChatCommandBuilder(char const* name, TypedHandler& handler, AcoreStrings help, uint32 securityLevel, Firelands::ChatCommands::Console allowConsole)
            : _name{ ASSERT_NOTNULL(name) }, _data{ std::in_place_type<InvokerEntry>, handler, help, securityLevel, allowConsole } { }

        template <typename TypedHandler>
        ChatCommandBuilder(char const* name, TypedHandler& handler, uint32 securityLevel, Firelands::ChatCommands::Console allowConsole)
    ...
    FC_GAME_API void LoadCommandMap();
    FC_GAME_API void InvalidateCommandMap();
    FC_GAME_API bool TryExecuteCommand(ChatHandler& handler, std::string_view cmd);
    FC_GAME_API void SendCommandHelpFor(ChatHandler& handler, std::string_view cmd);
    FC_GAME_API std::vector<std::string> GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd);
}

// backwards compatibility with old patches
using ChatCommand [[deprecated("std::vector<ChatCommand> should be ChatCommandTable! (using namespace Firelands::ChatCommands)")]] = Firelands::ChatCommands::ChatCommandBuilder;
