...
using ChatSubCommandMap = std::map<std::string_view, Firelands::Impl::ChatCommands::ChatCommandNode, StringCompareLessI_T>;

void Firelands::Impl::ChatCommands::ChatCommandNode::LoadFromBuilder(ChatCommandBuilder const& builder)
{
...
/*static*/ void Firelands::Impl::ChatCommands::ChatCommandNode::LoadCommandsIntoMap(ChatCommandNode* blank,
    std::map<std::string_view, Firelands::Impl::ChatCommands::ChatCommandNode, StringCompareLessI_T>& map, Firelands::ChatCommands::ChatCommandTable const& commands)
{
    for (ChatCommandBuilder const& builder : commands)
    {
        ...
        {
            std::vector<std::string_view> const tokens = Firelands::Tokenize(builder._name, COMMAND_DELIMITER, false);
...
/*static*/ ChatSubCommandMap const& Firelands::Impl::ChatCommands::ChatCommandNode::GetTopLevelMap()
{
...
/*static*/ void Firelands::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap()
{
    COMMAND_MAP.clear();
}

/*static*/ void Firelands::Impl::ChatCommands::ChatCommandNode::LoadCommandMap()
{
    ...
    if (PreparedQueryResult result = WorldDatabase.Query(WorldDatabase.GetPreparedStatement(WORLD_SEL_COMMANDS)))
    {
        do
        {
            ...
            for (std::string_view key : Firelands::Tokenize(name, COMMAND_DELIMITER, false))
            {
...
void Firelands::Impl::ChatCommands::ChatCommandNode::ResolveNames(std::string name)
{
...
static void LogCommandUsage(WorldSession const& session, std::string_view cmdStr)
{
    ...
    std::string logMessage = Firelands::StringFormat("Command: {} [Player: {} ({}) (Account: {}) X: {} Y: {} Z: {} Map: {} ({}) Area: {} ({}) Zone: {} ({}) Selected: {} ({})]",
...
void Firelands::Impl::ChatCommands::ChatCommandNode::SendCommandHelp(ChatHandler& handler) const
{
...
/*static*/ bool Firelands::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(ChatHandler& handler, std::string_view cmdStr)
{
...
/*static*/ void Firelands::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(ChatHandler& handler, std::string_view cmdStr)
{
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();

    for (std::string_view token : Firelands::Tokenize(cmdStr, COMMAND_DELIMITER, false))
    {
        FilteredCommandListIterator it1(*map, handler, token);
        if (!it1)
...
/*static*/ std::vector<std::string> Firelands::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmdStr)
{
    std::string path;
    ChatCommandNode const* cmd = nullptr;
    ChatSubCommandMap const* map = &GetTopLevelMap();
    ...
    while (!oldTail.empty())
    {
        ...
        { /* ok, so it1 points at a partially matching subcommand - let's see if there are others */
            ...
            { /* there are multiple matching subcommands - terminate here and show possibilities */
                ...
                auto possibility = ([prefix = std::string_view(path), suffix = std::string_view(newTail)](std::string_view match)
                {
                    if (prefix.empty())
                    {
                        return Firelands::StringFormat("{}{}{}", match, COMMAND_DELIMITER, suffix);
                    }
                    else
                    {
                        return Firelands::StringFormat("{}{}{}{}{}", prefix, COMMAND_DELIMITER, match, COMMAND_DELIMITER, suffix);
                    }
                });
        ...
        {
            path = Firelands::StringFormat("{}{}{}", path, COMMAND_DELIMITER, it1->first);
        }
        cmd = &it1->second;
        map = &cmd->_subCommands;
    ...
    { /* there is some trailing text, leave it as is */
        ...
        { /* if we matched a command at some point, auto-complete it */
            return { Firelands::StringFormat("{}{}{}", path, COMMAND_DELIMITER, oldTail) };
        }
        else
            return {};
    }
    ...
    { /* offer all subcommands */
        auto possibility = ([prefix = std::string_view(path)](std::string_view match)
        {
            ...
            {
                return Firelands::StringFormat("{}{}{}", prefix, COMMAND_DELIMITER, match);
            }
        });
...
bool Firelands::Impl::ChatCommands::ChatCommandNode::IsInvokerVisible(ChatHandler const& who) const
{
    if (!_invoker)
        return false;
    ...
    if (who.IsConsole() && (_permission.AllowConsole == Firelands::ChatCommands::Console::No))
        return false;

    if (who.IsConsole() && (_permission.AllowConsole == Firelands::ChatCommands::Console::Yes))
        return true;

    return !who.IsConsole() && who.IsAvailable(_permission.RequiredLevel);
}

bool Firelands::Impl::ChatCommands::ChatCommandNode::HasVisibleSubCommands(ChatHandler const& who) const
{
    for (auto it = _subCommands.begin(); it != _subCommands.end(); ++it)
...
void Firelands::ChatCommands::LoadCommandMap() { Firelands::Impl::ChatCommands::ChatCommandNode::LoadCommandMap(); }
void Firelands::ChatCommands::InvalidateCommandMap() { Firelands::Impl::ChatCommands::ChatCommandNode::InvalidateCommandMap(); }
bool Firelands::ChatCommands::TryExecuteCommand(ChatHandler& handler, std::string_view cmd) { return Firelands::Impl::ChatCommands::ChatCommandNode::TryExecuteCommand(handler, cmd); }
void Firelands::ChatCommands::SendCommandHelpFor(ChatHandler& handler, std::string_view cmd) { Firelands::Impl::ChatCommands::ChatCommandNode::SendCommandHelpFor(handler, cmd); }
std::vector<std::string> Firelands::ChatCommands::GetAutoCompletionsFor(ChatHandler const& handler, std::string_view cmd) { return Firelands::Impl::ChatCommands::ChatCommandNode::GetAutoCompletionsFor(handler, cmd); }
