...
namespace Firelands::Impl::ChatCommands
{
    ...
    struct ArgInfo { static_assert(Firelands::dependant_false_v<T>, "Invalid command parameter type - see ChatCommandArgs.h for possible types"); };
        ...
        static ChatCommandResult TryConsume(std::wstring& val, ChatHandler const* handler, std::string_view args)
        {
            ...
            if (next)
            {
                if (Utf8toWStr(utf8view, val))
                    ...
                    return GetFirelandsString(handler, LANG_CMDPARSER_INVALID_UTF8);
            }
            else
                return std::nullopt;
        }
    ...
    struct ArgInfo<Firelands::ChatCommands::Variant<Ts...>>
    {
        using V = std::variant<Ts...>;
        static constexpr std::size_t N = std::variant_size_v<V>;

        template <std::size_t I>
        static ChatCommandResult TryAtIndex([[maybe_unused]] Firelands::ChatCommands::Variant<Ts...>& val, [[maybe_unused]] ChatHandler const* handler, [[maybe_unused]] std::string_view args)
        {
            if constexpr (I < N)
            {
                ...
                else
                {
                    ...
                    if (StringStartsWith(nestedResult.GetErrorMessage(), "\""))
                        return Firelands::StringFormat("\"{}\"\n{} {}", thisResult.GetErrorMessage(), GetFirelandsString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
                    else
                        return Firelands::StringFormat("\"{}\"\n{} \"{}\"", thisResult.GetErrorMessage(), GetFirelandsString(handler, LANG_CMDPARSER_OR), nestedResult.GetErrorMessage());
                }
            }
        ...
        static ChatCommandResult TryConsume(Firelands::ChatCommands::Variant<Ts...>& val, ChatHandler const* handler, std::string_view args)
        {
            ChatCommandResult result = TryAtIndex<0>(val, handler, args);
            if (result.HasErrorMessage() && (result.GetErrorMessage().find('\n') != std::string::npos))
                return Firelands::StringFormat("{} {}", GetFirelandsString(handler, LANG_CMDPARSER_EITHER), result.GetErrorMessage());
            return result;
        }
    };
    ...
    template <>
    struct FC_GAME_API ArgInfo<ItemTemplate const*>
    {
    ...
    template <>
    struct FC_GAME_API ArgInfo<SpellInfo const*>
    {
    ...
    template <>
    struct FC_GAME_API ArgInfo<Quest const*>
    {
