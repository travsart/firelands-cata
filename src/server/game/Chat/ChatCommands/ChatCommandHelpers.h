...
namespace Firelands::Impl::ChatCommands
{
    ...
    FC_GAME_API std::string GetFirelandsString(ChatHandler const* handler, FirelandsStrings which);
    template <typename... Ts>
    std::string FormatAcoreString(ChatHandler const* handler, FirelandsStrings which, Ts&&... args)
    {
        return Firelands::StringFormat(GetFirelandsString(handler, which), std::forward<Ts>(args)...);
    }
}
