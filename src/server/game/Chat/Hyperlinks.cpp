...
using namespace Firelands::Hyperlinks;

inline uint8 toHex(char c) { return (c >= '0' && c <= '9') ? c - '0' + 0x10 : (c >= 'a' && c <= 'f') ? c - 'a' + 0x1a : 0x00; }

// Validates a single hyperlink
HyperlinkInfo Firelands::Hyperlinks::ParseSingleHyperlink(std::string_view str)
{
    uint32 color = 0;
    std::string_view tag;
...
struct LinkValidator<LinkTags::item>
{
    ...
    {
        ItemLocale const* locale = sObjectMgr->GetItemLocale(data.Item->GetId());

        char * const * randomSuffixes = nullptr;
        ...
        for (uint8 i = 0; i < TOTAL_LOCALES; ++i)
        {
            ...
            std::string_view name = data.Item->GetName(locale);
            if (name.empty())
                continue;
...
struct LinkValidator<LinkTags::enchant>
{
    static bool IsTextValid(SpellInfo const* info, std::string_view text)
    {
        ...
        for (auto pair = bounds.first; pair != bounds.second; ++pair)
        {
            ...
            // @todo support more locales
            std::string_view skillName = skill->DisplayName;
...
bool Firelands::Hyperlinks::CheckAllLinks(std::string_view str)
{
