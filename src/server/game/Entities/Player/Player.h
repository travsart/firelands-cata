...
// 2^n values, Player::m_isunderwater is a bitmask. These are Firelands internal
// values, they are never send to any client
...
struct PlayerSpell
{
    ...
    bool dependent : 1;  // learned as result another spell learn, skill grow,
                         // quest reward, etc
    bool
        disabled : 1;  // first rank has been learned in result talent learn but
...
uint32 const ArmorSpecializationIds[MAX_ARMOR_SPECIALIZATION_IDS] = {
    86530, 86531, 86529, 86528, 86525, 86524, 86526};
...
typedef std::unordered_map<uint32 /*instanceId*/, time_t /*releaseTime*/>
    InstanceTimeMap;
...
struct ActionButton
{
    ...
    void SetActionAndType(uint32 action, ActionButtonType type)
    {
        ...
        {
            packedData = newData;
            if (uState != ACTIONBUTTON_NEW) uState = ACTIONBUTTON_CHANGED;
        }
    }
};
...
struct PvPInfo
{
    PvPInfo()
        : IsHostile(false),
          ...
          IsInNoPvPArea(false),
          IsInFFAPvPArea(false),
          EndTimer(0)
    {
    }

    bool IsHostile;
    bool IsInHostileArea;  ///> Marks if player is in an area which forces PvP
                           ///flag
    bool IsInNoPvPArea;    ///> Marks if player is in a sanctuary or friendly
                           ///capital city
    bool IsInFFAPvPArea;   ///> Marks if player is in an FFAPvP area (such as
                           ///Gurubashi Arena)
    time_t EndTimer;       ///> Time when player unflags himself for PvP (flag
...
enum RuneCooldowns
{
    ...
    RUNE_MISS_COOLDOWN =
        1500  // cooldown applied on runes when the spell misses
};
...
{
    EnchantDuration()
        : item(nullptr), slot(MAX_ENCHANTMENT_SLOT), leftduration(0)
    {
    }
    EnchantDuration(Item* _item, EnchantmentSlot _slot, uint32 _leftduration)
        : item(_item), slot(_slot), leftduration(_leftduration)
    {
        ASSERT(item);
    }
...
enum PlayerFlags
{
    ...
    PLAYER_FLAGS_UNK7 =
        0x00000080,  // pre-3.0.3 PLAYER_FLAGS_FFA_PVP flag for FFA PVP state
    PLAYER_FLAGS_CONTESTED_PVP =
        0x00000100,  // Player has been involved in a PvP combat and will be
                     // attacked by contested guards
    ...
    PLAYER_FLAGS_UNK16 = 0x00010000,  // pre-3.0.3 PLAYER_FLAGS_SANCTUARY flag
                                      // for player entered sanctuary
    ...
    PLAYER_FLAGS_DISABLE_CASTING_EXCEPT_ABILITIES =
        0x00800000,  // disables casting any ability except for the ones
                     // specified in the aura effect's SpellClassMask
    PLAYER_FLAGS_DISABLE_ATTACKING_EXCEPT_ABILITIES =
        0x01000000,  // disables all attacks including ability casts except for
                     // the ones specified in the aura effect's SpellClassMask
    PLAYER_FLAGS_NO_XP_GAIN = 0x02000000,
    PLAYER_FLAGS_UNK26 = 0x04000000,
    PLAYER_FLAGS_AUTO_DECLINE_GUILD =
        0x08000000,  // Automatically declines guild invites
    PLAYER_FLAGS_GUILD_LEVEL_ENABLED =
        0x10000000,  // Lua_GetGuildLevelEnabled() - enables guild leveling
                     // related UI
...
static_assert((PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID & 1) == 0,
    "PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID must be aligned to 2 byte "
    "boundary");

#define PLAYER_BYTES_2_OVERRIDE_SPELLS_UINT16_OFFSET \
    (PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID / 2)
...
enum PlayerExtraFlags
{
    ...
    PLAYER_EXTRA_LEVEL_BOOSTED              = 0x0800,       // reserved
};
...
enum TransferAbortReason
{
    ...
    TRANSFER_ABORT_TOO_MANY_INSTANCES =
        0x04,  // You have entered too many instances recently.
    TRANSFER_ABORT_ZONE_IN_COMBAT =
        0x06,  // Unable to zone in while an encounter is in progress.
    TRANSFER_ABORT_INSUF_EXPAN_LVL =
        0x07,  // You must have <TBC, WotLK> expansion installed to access this
               // area.
    TRANSFER_ABORT_DIFFICULTY = 0x08,  // <Normal, Heroic, Epic> difficulty mode
                                       // is not available for %s.
    TRANSFER_ABORT_UNIQUE_MESSAGE =
        0x09,  // Until you've escaped TLK's grasp, you cannot leave this place!
    TRANSFER_ABORT_TOO_MANY_REALM_INSTANCES =
        0x0A,  // Additional instances cannot be launched, please try again
               // later.
    ...
    TRANSFER_ABORT_REALM_ONLY =
        0x0F,  // All players on party must be from the same realm.
    TRANSFER_ABORT_MAP_NOT_ALLOWED =
        0x10,  // Map can't be entered at this time.
...
enum InstanceResetWarningType
{
    ...
    RAID_INSTANCE_WARNING_MIN_SOON =
        3,  // WARNING! %s is scheduled to reset in %d minute(s). Please exit
            // the zone or you will be returned to your bind location!
...
enum PlayerChatTag
{
    ...
    CHAT_TAG_DEV = 0x10,
    CHAT_TAG_BOSS_SOUND =
        0x20,  // Plays "RaidBossEmoteWarning" sound on raid boss emote/whisper
...
enum PlayerDelayedOperations
{
    ...
    DELAYED_BG_MOUNT_RESTORE =
        0x08,  ///< Flag to restore mount state after teleport from BG
    DELAYED_BG_TAXI_RESTORE =
        0x10,  ///< Flag to restore taxi state after teleport from BG
    DELAYED_BG_GROUP_RESTORE =
        0x20,  ///< Flag to restore group state after teleport from BG
    DELAYED_END
};
...
FC_GAME_API extern uint64 const MAX_MONEY_AMOUNT;
...
{
    InstanceSave* save;
    /* permanent PlayerInstanceBinds are created in Raid/Heroic instances for
    players that aren't already permanently bound when they are inside when a
    boss is killed or when they enter an instance that the group leader is
    permanently bound to. */
    bool perm;
    /* extend state listing:
    EXPIRED  - doesn't affect anything unless manually re-extended by player
    NORMAL   - standard state
    EXTENDED - won't be promoted to EXPIRED at next reset period, will instead
    be promoted to NORMAL */
    BindExtensionState extendState;

    InstancePlayerBind()
        : save(nullptr), perm(false), extendState(EXTEND_STATE_NORMAL)
    {
    }
};
...
enum CharDeleteMethod
{
    ...
    CHAR_DELETE_UNLINK =
        1  // The character gets unlinked from the account,
...
struct BGData
{
    ...
    uint32 bgInstanceID;  ///< This variable is set to bg->m_InstanceID,
                          ///  when player is teleported to BG - (it is
                          ///  battleground's GUID)
...
{
    VoidStorageItem()
        : ItemId(0), ItemEntry(0), ItemRandomPropertyId(), ItemSuffixFactor(0)
    {
    }
    VoidStorageItem(uint64 id, uint32 entry, ObjectGuid const& creator,
        ItemRandomEnchantmentId randomPropertyId, uint32 suffixFactor)
        : ItemId(id),
          ItemEntry(entry),
          CreatorGuid(creator),
...
struct PlayerTalentInfo
{
          ...
          SpecsCount(1)
    {
        for (uint8 i = 0; i < MAX_TALENT_SPECS; ++i)
        {
            ...
            memset(
                SpecInfo[i].Glyphs, 0, MAX_GLYPH_SLOT_INDEX * sizeof(uint32));
    ...
    ~PlayerTalentInfo()
    {
        for (uint8 i = 0; i < MAX_TALENT_SPECS; ++i)
        {
            for (PlayerTalentMap::const_iterator itr =
                     SpecInfo[i].Talents->begin();
                 itr != SpecInfo[i].Talents->end(); ++itr)
                delete itr->second;
...
class FC_GAME_API Player : public Unit, public GridObject<Player>
{
   ...
   public:
    ...
    bool IsImmunedToSpellEffect(
        SpellInfo const* spellInfo, uint32 index, Unit* caster) const override;
    ...
    void SendTransferAborted(
        uint32 mapid, TransferAbortReason reason, uint8 arg = 0) const;
    void SendInstanceResetWarning(
        uint32 mapid, Difficulty difficulty, uint32 time, bool welcome) const;

    bool CanInteractWithQuestGiver(Object* questGiver) const;
    Creature* GetNPCIfCanInteractWith(
        ObjectGuid const& guid, uint32 npcflagmask) const;
    GameObject* GetGameObjectIfCanInteractWith(ObjectGuid const& guid) const;
    GameObject* GetGameObjectIfCanInteractWith(
        ObjectGuid const& guid, GameobjectTypes type) const;
    ...
    uint32 GetBarberShopCost(uint8 newhairstyle, uint8 newhaircolor,
        uint8 newfacialhair,
        BarberShopStyleEntry const* newSkin = nullptr) const;
    ...
    bool ActivateTaxiPathTo(std::vector<uint32> const& nodes,
        Creature* npc = nullptr, uint32 spellid = 0);
    ...
    Pet* GetPet() const;
    Pet* SummonPet(uint32 entry, float x, float y, float z, float ang,
        PetType petType, uint32 despwtime);
    ...
    /// Handles said message in regular chat based on declared language and in
    /// config pre-defined Range.
    void Say(std::string const& text, Language language,
        WorldObject const* = nullptr) override;
    void Say(uint32 textId, WorldObject const* target = nullptr) override;
    /// Handles yelled message in regular chat based on declared language and in
    /// config pre-defined Range.
    void Yell(std::string const& text, Language language,
        WorldObject const* = nullptr) override;
    void Yell(uint32 textId, WorldObject const* target = nullptr) override;
    /// Outputs an universal text which is supposed to be an action.
    void TextEmote(std::string const& text, WorldObject const* = nullptr,
        bool = false) override;
    void TextEmote(uint32 textId, WorldObject const* target = nullptr,
        bool isBossEmote = false) override;
    /// Handles whispers from Addons and players based on sender, receiver's
    /// guid and language.
    void Whisper(std::string const& text, Language language, Player* receiver,
        bool = false) override;
    void Whisper(
        uint32 textId, Player* target, bool isBossWhisper = false) override;
    void WhisperAddon(
        std::string const& text, std::string const& prefix, Player* receiver);
    ...
    uint8 FindEquipSlot(
        ItemTemplate const* proto, uint32 slot, bool swap) const;
    uint32 GetItemCount(
        uint32 item, bool inBankAlso = false, Item* skipItem = nullptr) const;
    uint32 GetItemCountWithLimitCategory(
        uint32 limitCategory, Item* skipItem = nullptr) const;
    ...
    Item* GetWeaponForAttack(
        WeaponAttackType attackType, bool useable = false) const;
    Item* GetShield(bool useable = false) const;
    static WeaponAttackType GetAttackBySlot(
        uint8 slot);  // MAX_ATTACK if not weapon slot
    ...
    {
        return GetByteValue(
            PLAYER_BYTES_2, PLAYER_BYTES_2_OFFSET_BANK_BAG_SLOTS);
    }
    void SetBankBagSlotCount(uint8 count)
    {
        SetByteValue(
            PLAYER_BYTES_2, PLAYER_BYTES_2_OFFSET_BANK_BAG_SLOTS, count);
    }
    bool HasItemCount(
        uint32 item, uint32 count = 1, bool inBankAlso = false) const;
    bool HasItemFitToSpellRequirements(
        SpellInfo const* spellInfo, Item const* ignoreItem = nullptr) const;
    bool HasAllItemsToFitToSpellRequirements(SpellInfo const* spellInfo);
    bool CanNoReagentCast(SpellInfo const* spellInfo) const;
    bool HasItemOrGemWithIdEquipped(
        uint32 item, uint32 count, uint8 except_slot = NULL_SLOT) const;
    bool HasItemWithLimitCategoryEquipped(uint32 limitCategory, uint32 count,
        uint8 except_slot = NULL_SLOT) const;
    bool HasGemWithLimitCategoryEquipped(uint32 limitCategory, uint32 count,
        uint8 except_slot = NULL_SLOT) const;
    InventoryResult CanTakeMoreSimilarItems(
        Item* pItem, uint32* offendingItemId = nullptr) const;
    InventoryResult CanTakeMoreSimilarItems(
        uint32 entry, uint32 count, uint32* offendingItemId = nullptr) const
    {
        return CanTakeMoreSimilarItems(
            entry, count, nullptr, nullptr, offendingItemId);
    }
    InventoryResult CanStoreNewItem(uint8 bag, uint8 slot,
        ItemPosCountVec& dest, uint32 item, uint32 count,
        uint32* no_space_count = nullptr) const;
    InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest,
        Item* pItem, bool swap = false) const;
    InventoryResult CanStoreItems(
        Item** items, int count, uint32* offendingItemId) const;
    InventoryResult CanEquipNewItem(
        uint8 slot, uint16& dest, uint32 item, bool swap) const;
    InventoryResult CanEquipItem(uint8 slot, uint16& dest, Item* pItem,
        bool swap, bool not_loading = true) const;

    InventoryResult CanEquipUniqueItem(Item* pItem,
        uint8 except_slot = NULL_SLOT, uint32 limit_count = 1) const;
    InventoryResult CanEquipUniqueItem(ItemTemplate const* itemProto,
        uint8 except_slot = NULL_SLOT, uint32 limit_count = 1) const;
    InventoryResult CanUnequipItems(uint32 item, uint32 count) const;
    InventoryResult CanUnequipItem(uint16 src, bool swap) const;
    InventoryResult CanBankItem(uint8 bag, uint8 slot, ItemPosCountVec& dest,
        Item* pItem, bool swap, bool not_loading = true) const;
    InventoryResult CanUseItem(Item* pItem, bool not_loading = true) const;
    bool HasItemTotemCategory(uint32 TotemCategory) const;
    InventoryResult CanUseItem(ItemTemplate const* pItem) const;
    InventoryResult CanUseAmmo(uint32 item) const;
    InventoryResult CanRollForItemInLFG(
        ItemTemplate const* item, WorldObject const* lootedObject) const;
    Item* StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update,
    ...
    void AutoStoreLoot(uint8 bag, uint8 slot, uint32 loot_id,
        LootStore const& store, bool broadcast = false);
    void AutoStoreLoot(
        uint32 loot_id, LootStore const& store, bool broadcast = false)
    {
    ...
    InventoryResult CanTakeMoreSimilarItems(uint32 entry, uint32 count,
        Item* pItem, uint32* no_space_count = nullptr,
        uint32* offendingItemId = nullptr) const;
    InventoryResult CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest,
        uint32 entry, uint32 count, Item* pItem = nullptr, bool swap = false,
        uint32* no_space_count = nullptr) const;
    ...
    */
    void ModifyCurrency(uint32 id, int32 count, bool printLog = true,
        bool ignoreMultipliers = false, bool isRefund = false);
    ...
    void MoveItemToInventory(ItemPosCountVec const& dest, Item* pItem,
        bool update, bool in_characterInventoryDB = false);
    // in trade, guild bank, mail....
    void RemoveItemDependentAurasAndCasts(Item* pItem);
    void DestroyItem(uint8 bag, uint8 slot, bool update);
    void DestroyItemCount(
        uint32 item, uint32 count, bool update, bool unequip_check = false);
    ...
    void SendBuyError(
        BuyResult msg, Creature* creature, uint32 item, uint32 param) const;
    void SendSellError(
        SellResult msg, Creature* creature, ObjectGuid guid) const;
    ...
    void SendNewItem(Item* item, uint32 count, bool received, bool created,
        bool broadcast = false, bool sendChatMessage = true);
    bool BuyItemFromVendorSlot(ObjectGuid vendorguid, uint32 vendorslot,
        uint32 item, uint32 count, uint8 bag, uint8 slot);
    bool BuyCurrencyFromVendorSlot(ObjectGuid vendorGuid, uint32 vendorSlot,
        uint32 currency, uint32 count);
    bool _StoreOrEquipNewItem(uint32 vendorslot, uint32 item, uint32 count,
        uint8 bag, uint8 slot, int64 price, ItemTemplate const* pProto,
        Creature* pVendor, VendorItem const* crItem, bool bStore);
    ...
    void AddEnchantmentDuration(
        Item* item, EnchantmentSlot slot, uint32 duration);
    void ApplyEnchantment(Item* item, EnchantmentSlot slot, bool apply,
        bool apply_dur = true, bool ignore_condition = false);
    void ApplyEnchantment(Item* item, bool apply);
    void ApplyReforgeEnchantment(Item* item, bool apply);
    void UpdateSkillEnchantments(
        uint16 skill_id, uint16 curr_value, uint16 new_value);
    ...
    void PrepareGossipMenu(
        WorldObject* source, uint32 menuId = 0, bool showQuests = false);
    void SendPreparedGossip(WorldObject* source);
    void OnGossipSelect(
        WorldObject* source, uint32 gossipListId, uint32 menuId);
    ...
    {
        return quest && (quest->GetQuestLevel() > 0) ? quest->GetQuestLevel()
                                                     : getLevel();
    }
    ...
    void RewardQuest(Quest const* quest, uint32 reward, Object* questGiver,
        bool announce = true);
    ...
    bool HasQuestForItem(
        uint32 itemId, uint32 excludeQuestId = 0, bool turnIn = false) const;
    ...
    void SendQuestReward(
        Quest const* quest, Creature const* questGiver, uint32 xp);
    void SendQuestFailed(
        uint32 questId, InventoryResult reason = EQUIP_ERR_OK) const;
    ...
    void SendQuestUpdateAddCredit(Quest const* quest, ObjectGuid guid,
    ...
    {
        m_playerSharingQuest = guid;
        m_sharedQuestId = id;
    }
    void ClearQuestSharingInfo()
    {
        m_playerSharingQuest = ObjectGuid::Empty;
        m_sharedQuestId = 0;
    }

    // Returns the ID of the next available quest in chain of the most recently
    // rewarded quest that has been rewarded via UI popup
    ...
    static bool LoadPositionFromDB(uint32& mapid, float& x, float& y, float& z,
        float& o, bool& in_flight, ObjectGuid guid);

    static bool IsValidGender(uint8 Gender) { return Gender <= GENDER_FEMALE; }
    static bool ValidateAppearance(uint8 race, uint8 class_, uint8 gender,
        uint8 hairID, uint8 hairColor, uint8 faceID, uint8 facialHair,
        uint8 skinColor, bool create = false);
    static bool IsValidClass(uint8 Class)
    {
        return ((1 << (Class - 1)) & CLASSMASK_ALL_PLAYABLE) != 0;
    }
    static bool IsValidRace(uint8 Race)
    {
        return ((1 << (Race - 1)) & RACEMASK_ALL_PLAYABLE) != 0;
    }
    ...
    void SaveInventoryAndGoldToDB(CharacterDatabaseTransaction&
            trans);  // fast save function for item/money cheating preventing
    void SaveGoldToDB(CharacterDatabaseTransaction& trans) const;

    static void SetUInt32ValueInArray(
        Tokenizer& data, uint16 index, uint32 value);
    static void Customize(CharacterCustomizeInfo const* customizeInfo,
        CharacterDatabaseTransaction& trans);
    static void SavePositionInDB(WorldLocation const& loc, uint16 zoneId,
        ObjectGuid guid, CharacterDatabaseTransaction& trans);
    ...
    void SendMailResult(uint32 mailId, MailResponseType mailAction,
        ...
        ObjectGuid::LowType item_guid = 0, uint32 item_count = 0) const;
    ...
    void SendItemRetrievalMail(uint32 itemEntry,
        uint32 count);  // Item retrieval mails sent by The Postmaster (34337),
    ...
    void RemoveSpell(
        uint32 spell_id, bool disabled = false, bool learn_low_rank = true);
    ...
    static bool IsAffectedBySpellmod(
        SpellInfo const* spellInfo, SpellModifier* mod, Spell* spell = nullptr);
    template <class T>
    void ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue,
    ...
    void SetResurrectRequestData(
        Unit* caster, uint32 health, uint32 mana, uint32 appliedAura);
    ...
    {
        if (!IsResurrectRequested()) return false;
    ...
    static void RemoveFromGroup(Group* group, ObjectGuid guid,
        ...
        ObjectGuid kicker = ObjectGuid::Empty, char const* reason = nullptr);
    ...
    {
        SetUInt32Value(PLAYER_GUILDLEVEL, level);
    }
    ...
    void SetArenaTeamInfoField(
        uint8 slot, ArenaTeamInfoType type, uint32 value);
    ...
    void CalculateMinMaxDamage(WeaponAttackType attType, bool normalized,
        bool addTotalPct, float& minDamage, float& maxDamage) const override;
    ...
    /// Returns base spellpower bonus from spellpower stat on items, without
    /// spellpower from intellect stat
    ...
    bool UpdatePosition(float x, float y, float z, float orientation,
        bool teleport = false) override;
    bool UpdatePosition(Position const& pos, bool teleport = false) override
    {
        return UpdatePosition(pos.GetPositionX(), pos.GetPositionY(),
            pos.GetPositionZ(), pos.GetOrientation(), teleport);
    }
    void ProcessTerrainStatusUpdate(ZLiquidStatus oldLiquidStatus,
        Optional<LiquidData> const& newLiquidData) override;
    ...
    void SendMessageToSetInRange(
        WorldPacket const* data, float dist, bool self) const override;
    void SendMessageToSetInRange(WorldPacket const* data, float dist, bool self,
        bool own_team_only) const;
    void SendMessageToSet(
        WorldPacket const* data, Player const* skipped_rcvr) const override;

    Corpse* GetCorpse() const;
    void SpawnCorpseBones(bool triggerSave = true);
    Corpse* CreateCorpse();
    void KillPlayer();
    static void OfflineResurrect(
        ObjectGuid const& guid, CharacterDatabaseTransaction& trans);
    bool HasCorpse() const
    {
        return _corpseLocation.GetMapId() != MAPID_INVALID;
    }
    ...
    bool CanJoinConstantChannelInZone(
        ChatChannelsEntry const* channel, AreaTableEntry const* zone) const;
    ...
    int32 CalculateReputationGain(ReputationSource source,
        uint32 creatureOrQuestLevel, int32 rep, int32 faction,
        bool noQuestBonus = false);

    void UpdateSkillsForLevel();
    void ModifySkillBonus(uint32 skillid, int32 val, bool talent);

    void SetLastSoulburnSpell(SpellInfo const* spell)
    {
        m_lastSoulburnSpell = spell;
    }
    SpellInfo const* GetLastSoulburnSpell() const
    {
        return m_lastSoulburnSpell;
    }
    ...
    bool RewardHonor(Unit* victim, uint32 groupsize, int32 honor = -1,
        bool pvptoken = false);
    ...
    {
        return GetByteValue(PLAYER_BYTES_3, PLAYER_BYTES_3_OFFSET_INEBRIATION);
    }
    ...
    {
        return GetUInt32Value(PLAYER_SHIELD_BLOCK);
    }
    ...
    void HandleBaseModFlatValue(
        BaseModGroup modGroup, float amount, bool apply);
    ...
    bool CheckAttackFitToAuraRequirement(
        WeaponAttackType attackType, AuraEffect const* aurEff) const override;

    void _ApplyItemMods(
        Item* item, uint8 slot, bool apply, bool updateItemAuras = true);
    void _RemoveAllItemMods();
    void _ApplyAllItemMods();
    void _ApplyAllLevelScaleItemMods(bool apply);
    void _ApplyItemBonuses(ItemTemplate const* proto, uint8 slot, bool apply,
        bool only_level_scale = false);
    void _ApplyWeaponDamage(uint8 slot, ItemTemplate const* proto,
        ScalingStatValuesEntry const* ssv, bool apply);
    bool EnchantmentFitsRequirements(
        uint32 enchantmentcondition, int8 slot) const;
    void ToggleMetaGemsActive(uint8 exceptslot, bool apply);
    void CorrectMetaGemEnchants(uint8 slot, bool apply);
    void InitDataForForm(bool reapplyMods = false);

    void ApplyItemEquipSpell(Item* item, bool apply, bool form_change = false);
    void ApplyEquipSpell(SpellInfo const* spellInfo, Item* item, bool apply,
        bool form_change = false);
    void UpdateEquipSpellsAtFormChange();
    void CastItemCombatSpell(DamageInfo const& damageInfo);
    void CastItemCombatSpell(
        DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto);
    ...
    void SendUpdateWorldState(
        uint32 variable, uint32 value, bool hidden = false) const;
    ...
    uint32 GetBattlegroundQueueIndex(
        BattlegroundQueueTypeId bgQueueTypeId) const;
    bool IsInvitedForBattlegroundQueueType(
        BattlegroundQueueTypeId bgQueueTypeId) const;
    ...
    void SetInviteForBattlegroundQueueType(
    ...
    // returns true if the player is in active state for outdoor pvp objective
    ...
    bool SetDisableGravity(bool disable, bool packetOnly = false,
        bool updateAnimationTier = true) override;
    ...
    template <class T>
    void UpdateVisibilityOf(
        T* target, UpdateData& data, std::set<Unit*>& visibleNow);
    ...
    typedef std::unordered_map<uint32 /*mapId*/, InstancePlayerBind>
        BoundInstancesMap;
    ...
    InstancePlayerBind* GetBoundInstance(
        uint32 mapid, Difficulty difficulty, bool withExpired = false);
    InstancePlayerBind const* GetBoundInstance(
        uint32 mapid, Difficulty difficulty, bool withExpired = false) const;
    ...
    void UnbindInstance(
        uint32 mapid, Difficulty difficulty, bool unload = false);
    void UnbindInstance(BoundInstancesMap::iterator& itr, Difficulty difficulty,
        bool unload = false);
    InstancePlayerBind* BindToInstance(InstanceSave* save, bool permanent,
    ...
    bool Satisfy(
        AccessRequirement const* ar, uint32 target_map, bool report = false);
    ...
    {
        return const_cast<Group const*>(m_group.getTarget());
    }
    ...
    PartyResult CanUninviteFromGroup(
        ObjectGuid guidMember = ObjectGuid::Empty) const;
    ...
    void SetLastUsedRuneIndex(uint8 index)
    {
        m_runes->lastUsedRuneMask |= (1 << index);
    }
    ...
    void AddRuneByAuraEffect(uint8 index, RuneType newType,
        AuraEffect const* aura, AuraType auraType, SpellInfo const* spellInfo);
    ...
    void ResetAchievementCriteria(AchievementCriteriaCondition condition,
        uint64 value, bool evenIfCriteriaComplete = false);
    void UpdateAchievementCriteria(AchievementCriteriaTypes type,
        uint64 miscValue1 = 0, uint64 miscValue2 = 0, uint64 miscValue3 = 0,
        Unit* unit = nullptr, GameObject* go = nullptr);
    void StartTimedAchievement(
        AchievementCriteriaTimedTypes type, uint32 entry, uint32 timeLost = 0);
    void RemoveTimedAchievement(
    ...
    /*! These methods send different packets to the client in apply and unapply
       case. These methods are only sent to the current unit.
    */
    ...
    {
        return HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_VOID_UNLOCKED);
    }
    ...
    */
    ...
    void _LoadMail(
        PreparedQueryResult mailsResult, PreparedQueryResult mailItemsResult);
    static Item* _LoadMailedItem(ObjectGuid const& playerGuid, Player* player,
    ...
    */
    ...
    uint32 m_lastPotionId;  // last used health/mana potion in combat, that
                            // block next potion use
   ...
   private:
    ...
    InventoryResult CanStoreItem_InSpecificSlot(uint8 bag, uint8 slot,
        ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count,
        bool swap, Item* pSrcItem) const;
    InventoryResult CanStoreItem_InBag(uint8 bag, ItemPosCountVec& dest,
        ItemTemplate const* pProto, uint32& count, bool merge,
        bool non_specialized, Item* pSrcItem, uint8 skip_bag,
        uint8 skip_slot) const;
    InventoryResult CanStoreItem_InInventorySlots(uint8 slot_begin,
        uint8 slot_end, ItemPosCountVec& dest, ItemTemplate const* pProto,
        uint32& count, bool merge, Item* pSrcItem, uint8 skip_bag,
        uint8 skip_slot) const;
    Item* _StoreItem(
        uint16 pos, Item* pItem, uint32 count, bool clone, bool update);
    Item* _LoadItem(CharacterDatabaseTransaction& trans, uint32 zoneId,
    ...
    void SendItemRefundResult(
        Item* item, ItemExtendedCostEntry const* iece, uint8 error);

    // know currencies are not removed at any point (0 displayed)
    void AddKnownCurrency(uint32 itemId);

    void AdjustQuestReqItemCount(
        Quest const* quest, QuestStatusData& questStatusData);
...
FC_GAME_API void AddItemsSetItem(Player* player, Item* item);
FC_GAME_API void RemoveItemsSetItem(Player* player, ItemTemplate const* proto);
