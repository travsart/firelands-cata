...
enum PlayerFlags
{
    ...
    PLAYER_FLAGS_CONTESTED_PVP = 0x00000100, // Player has been involved in a PvP combat and will be
    ...
    PLAYER_FLAGS_DISABLE_CASTING_EXCEPT_ABILITIES = 0x00800000,   // disables casting any ability except for the ones
                                                                  // specified in the aura effect's SpellClassMask
    PLAYER_FLAGS_DISABLE_ATTACKING_EXCEPT_ABILITIES = 0x01000000, // disables all attacks including ability casts except for
    ...
    PLAYER_FLAGS_GUILD_LEVEL_ENABLED = 0x10000000, // Lua_GetGuildLevelEnabled() - enables guild leveling
                                                   // related UI
...
static_assert((PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID & 1) == 0, "PLAYER_FIELD_BYTES_2_OFFSET_OVERRIDE_SPELLS_ID must be aligned to 2 byte "
                                                                         "boundary");
...
enum TransferAbortReason
{
    ...
    TRANSFER_ABORT_INSUF_EXPAN_LVL = 0x07,              // You must have <TBC, WotLK> expansion installed to access this
                                                        // area.
...
enum InstanceResetWarningType
{
    ...
    RAID_INSTANCE_WARNING_MIN_SOON = 3, // WARNING! %s is scheduled to reset in %d minute(s). Please exit
...
struct BGData
{
    BGData() : bgInstanceID(0), bgTypeID(BATTLEGROUND_TYPE_NONE), bgAfkReportedCount(0), bgAfkReportedTimer(0), bgTeam(0), mountSpell(0)
    {
...
struct VoidStorageItem
{
    ...
    VoidStorageItem(uint64 id, uint32 entry, ObjectGuid const& creator, ItemRandomEnchantmentId randomPropertyId, uint32 suffixFactor)
        : ItemId(id), ItemEntry(entry), CreatorGuid(creator), ItemRandomPropertyId(randomPropertyId), ItemSuffixFactor(suffixFactor)
    {
    }
...
{
    PlayerTalentInfo() : FreeTalentPoints(0), UsedTalentCount(0), QuestRewardedTalentCount(0), ResetTalentsCost(0), ResetTalentsTime(0), ActiveSpec(0), SpecsCount(1)
    {
    ...
    {
        TalentSpecInfo() : Talents(nullptr), TalentTree(0) { memset(Glyphs, 0, MAX_GLYPH_SLOT_INDEX * sizeof(uint32)); }
...
class FC_GAME_API Player : public Unit, public GridObject<Player>
{
  ...
  public:
    ...
    */
    ...
    template <class T> void ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell = nullptr) const;
    ...
    void ProcessTerrainStatusUpdate(LiquidStatus oldLiquidStatus, Optional<LiquidData> const& newLiquidData) override;
    ...
    template <class T> void UpdateVisibilityOf(T* target, UpdateData& data, std::set<Unit*>& visibleNow);
  ...
  protected:
    ...
    */
    ...
    {
        LFGRewardInfo(uint8 _completionsThisPeriod = 0, bool _isDaily = false) : CompletionsThisPeriod(_completionsThisPeriod), IsDaily(_isDaily) {}
  ...
  private:
    ...
    InventoryResult CanStoreItem_InBag(
        uint8 bag, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count, bool merge, bool non_specialized, Item* pSrcItem, uint8 skip_bag, uint8 skip_slot) const;
    InventoryResult CanStoreItem_InInventorySlots(
        uint8 slot_begin, uint8 slot_end, ItemPosCountVec& dest, ItemTemplate const* pProto, uint32& count, bool merge, Item* pSrcItem, uint8 skip_bag, uint8 skip_slot) const;
