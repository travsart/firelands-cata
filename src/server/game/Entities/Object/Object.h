  ...
  public:
    ...
    AreaTrigger* ToAreaTrigger()
    {
        if (GetTypeId() == TYPEID_AREATRIGGER)
            return reinterpret_cast<AreaTrigger*>(this);
        else
            return nullptr;
    }
    ...
    {
        if (GetTypeId() == TYPEID_AREATRIGGER)
            return reinterpret_cast<AreaTrigger const*>(this);
        else
            return nullptr;
    }
...
template <class T> class GridObject
{
  public:
    virtual ~GridObject() {}
...
template <class T_VALUES, class T_FLAGS, class FLAG_TYPE, size_t ARRAY_SIZE> class FlaggedValuesArray32
{
  public:
...
class FC_GAME_API WorldObject : public Object, public WorldLocation
{
  ...
  public:
    ...
    LiquidStatus GetLiquidStatus() const { return m_liquidStatus; }
    ...
    TempSummon* SummonCreature(
        uint32 entry, Position const& pos, TempSummonType despawnType = TEMPSUMMON_MANUAL_DESPAWN, uint32 despawnTime = 0, uint32 vehId = 0, ObjectGuid privateObjectOwner = ObjectGuid::Empty);
    TempSummon* SummonCreature(uint32 entry, Position const& pos, TempSummonType despawnType, Milliseconds despawnTime, uint32 vehId = 0, ObjectGuid privateObjectOwner = ObjectGuid::Empty)
    {
        return SummonCreature(entry, pos, despawnType, uint32(despawnTime.count()), vehId, privateObjectOwner);
    }
    TempSummon* SummonCreature(
        uint32 entry, float x, float y, float z, float o = 0, TempSummonType despawnType = TEMPSUMMON_MANUAL_DESPAWN, uint32 despawnTime = 0, ObjectGuid privateObjectOwner = ObjectGuid::Empty);
    ...
    template <typename Container> void GetGameObjectListWithEntryInGrid(Container& gameObjectContainer, uint32 entry, float maxSearchRange = 250.0f) const;

    template <typename Container> void GetCreatureListWithEntryInGrid(Container& creatureContainer, uint32 entry, float maxSearchRange = 250.0f) const;

    template <typename Container> void GetPlayerListInGrid(Container& playerContainer, float maxSearchRange) const;
...
namespace Firelands
{
    ...
    class ObjectDistanceOrderPred
    {
      public:
        ...
        bool operator()(WorldObject const* left, WorldObject const* right) const { return _refObj->GetDistanceOrder(left, right) == _ascending; }
