...
DamageInfo::DamageInfo(
    Unit* attacker, Unit* victim, uint32 damage, SpellInfo const* spellInfo, SpellSchoolMask schoolMask, DamageEffectType damageType, WeaponAttackType attackType, uint32 cleanDamage)
    : m_attacker(attacker), m_victim(victim), m_damage(damage), m_spellInfo(spellInfo), m_schoolMask(schoolMask), m_damageType(damageType), m_attackType(attackType), m_cleanDamage(cleanDamage),
...
DamageInfo::DamageInfo(CalcDamageInfo const& dmgInfo, uint8 damageIndex)
    : m_attacker(dmgInfo.attacker), m_victim(dmgInfo.target), m_damage(dmgInfo.damages[damageIndex].damage), m_spellInfo(nullptr),
      m_schoolMask(SpellSchoolMask(dmgInfo.damages[damageIndex].damageSchoolMask)), m_damageType(DIRECT_DAMAGE), m_attackType(dmgInfo.attackType), m_cleanDamage(dmgInfo.cleanDamage),
      m_absorb(dmgInfo.damages[damageIndex].absorb), m_resist(dmgInfo.damages[damageIndex].resist), m_block(dmgInfo.blocked_amount)
{
    ...
    bool const damageNullified = (dmgInfo.HitInfo & (HITINFO_FULL_ABSORB | HITINFO_FULL_RESIST)) != 0 || (m_hitMask & (PROC_HIT_IMMUNE | PROC_HIT_FULL_BLOCK)) != 0;
...
ProcEventInfo::ProcEventInfo(Unit* actor, Unit* actionTarget, Unit* procTarget, uint32 typeMask, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell const* spell,
    DamageInfo* damageInfo, HealInfo* healInfo, SpellInfo const* triggeredByAuraSpell, int8 procAuraEffectIndex)
    : _actor(actor), _actionTarget(actionTarget), _procTarget(procTarget), _typeMask(typeMask), _spellTypeMask(spellTypeMask), _spellPhaseMask(spellPhaseMask), _hitMask(hitMask), _spell(spell),
      _damageInfo(damageInfo), _healInfo(healInfo), _triggeredByAuraSpell(triggeredByAuraSpell), _procAuraEffectIndex(procAuraEffectIndex)
{
    _chance.reset();
}
...
Unit::Unit(bool isWorldObject)
    : WorldObject(isWorldObject),
      ...
      m_lastSanctuaryTime(0), IsAIEnabled(false), NeedChangeAI(false), m_ControlledByPlayer(false), m_CreatedByPlayer(false), movespline(new Movement::MoveSpline()), i_AI(nullptr),
      i_disabledAI(nullptr), m_realRace(0), m_race(0), m_AutoRepeatFirstCast(false), m_procDeep(0), m_removedAurasCount(0), i_motionMaster(new MotionMaster(this)),
      m_regenTimer(UNIT_HEALTH_REGENERATION_INTERVAL), m_ThreatMgr(this), m_vehicle(nullptr), m_vehicleKit(nullptr), m_unitTypeMask(UNIT_MASK_NONE), m_HostileRefMgr(this), m_comboTarget(nullptr),
      m_comboPoints(0), m_spellHistory(new SpellHistory(this)), _scheduler(this), _isIgnoringCombat(false), m_interruptMask(SpellAuraInterruptFlags::None),
      m_interruptMask2(SpellAuraInterruptFlags2::None), m_Diminishing()
{
...
class SplineHandler
{
  public:
    ...
    {
        if ((result & (Movement::MoveSpline::Result_NextSegment | Movement::MoveSpline::Result_JustArrived)) && _unit->IsCreature() &&
            _unit->GetMotionMaster()->GetCurrentMovementGeneratorType() == ESCORT_MOTION_TYPE && _unit->movespline->GetId() == _unit->GetMotionMaster()->GetCurrentSplineId())
        {
...
bool Unit::IsWithinMeleeRange(Unit const* obj, float dist = 0.f) const
{
    if (!obj || !IsInMap(obj) || !IsInPhase(obj))
        return false;
...
{
    float range = GetCombatReach() + target->GetCombatReach() + 1.3333334f; // 4.0f / 3.0f
...
bool Unit::IsWithinBoundaryRadius(const Unit* obj) const
{
    ...
    return IsInDist(obj->GetPosition(), objBoundaryRadius);
}
...
bool Unit::HasBreakableByDamageCrowdControlAura(Unit* excludeCasterChannel) const
{
    ...
    return (HasBreakableByDamageAuraType(SPELL_AURA_MOD_CONFUSE, excludeAura) || HasBreakableByDamageAuraType(SPELL_AURA_MOD_FEAR, excludeAura) ||
            HasBreakableByDamageAuraType(SPELL_AURA_MOD_STUN, excludeAura) || HasBreakableByDamageAuraType(SPELL_AURA_MOD_ROOT, excludeAura) ||
...
/*static*/ uint32 Unit::DealDamage(Unit* attacker, Unit* victim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask,
    SpellInfo const* spellProto, bool durabilityLoss, bool /*allowGM*/, Spell const* damageSpell /*= nullptr*/)
{
    ...
    {
        // interrupting auras with SpellAuraInterruptFlags::Damage before checking !damage (absorbed damage breaks that type of
        // auras)
        if (spellProto)
        {
    ...
    {
        uint32 rage = uint32(float)((attacker->GetAttackTime(cleanDamage->attackType) / 1000.0f) * 6.5f); // In AC it is (cleanDamage->attackType == BASE_ATTACK ? 3.5f : 1.75f)
        ...
        case OFF_ATTACK:
            ...
            [[fallthrough]];
        case BASE_ATTACK:
            attacker->RewardRage(rage_damage, rage, true);
            break;
        default:
            break;
        }
    ...
    LOG_DEBUG("entities.unit", "%s dealt %u damage to %s", attacker->GetGUID().ToString().c_str(), damage, victim->GetGUID().ToString().c_str());
    ...
    if (victim->IsPlayer() && victim->ToPlayer()->duel && damage >= (health - 1))
    {
        ...
        if (victim->ToPlayer()->duel->Opponent == attacker || victim->ToPlayer()->duel->Opponent->GetGUID() == attacker->GetOwnerGUID()) // attacker->GetControllingPlayer()
            damage = health - 1;
    ...
    if (attacker && attacker != victim)
    {
        if (Player* killer = attacker->GetCharmerOrOwnerPlayerOrPlayerItself())
        {
            ...
            if (Battleground* bg = killer->GetBattleground())
            {
                ...
                killer->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_DEALT, damage); // AC commented this out
            }
        }
    }
    ...
    if (victim->IsPlayer())
    {
        victim->ToPlayer()->UpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_HIGHEST_HIT_RECEIVED, damage); // AC commented this out
    }
...
SpellCastResult Unit::CastSpell(SpellCastTargets const& targets, SpellInfo const* spellInfo, CustomSpellValues const* value, TriggerCastFlags triggerFlags, Item* castItem,
    AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    if (!spellInfo)
    {
...
SpellCastResult Unit::CastSpell(Unit* victim, uint32 spellId, TriggerCastFlags triggerFlags /*= TRIGGER_NONE*/, Item* castItem /*= nullptr*/, AuraEffect const* triggeredByAura /*= nullptr*/,
...
SpellCastResult Unit::CastSpell(
    Unit* victim, SpellInfo const* spellInfo, bool triggered, Item* castItem /*= nullptr*/, AuraEffect const* triggeredByAura /*= nullptr*/, ObjectGuid originalCaster /*= ObjectGuid::Empty*/)
{
...
SpellCastResult Unit::CastSpell(WorldObject* target, uint32 spellId, CastSpellExtraArgs const& args)
{
    SpellCastTargets targets;
    if (target)
    {
        ...
        {
            LOG_ERROR("entities.unit", "CastSpell: Invalid target %s passed to spell cast by %s", target->GetGUID().ToString().c_str(), GetGUID().ToString().c_str());
    ...
    return CastSpell(targets, spellInfo, args.mod, args.TriggerFlags, args.CastItem, args.TriggeringAura, args.OriginalCaster);
}

SpellCastResult Unit::CastSpell(Position const& dest, uint32 spellId, CastSpellExtraArgs const& args)
{
    SpellCastTargets targets;
    targets.SetDst(dest);
...
SpellCastResult Unit::CastCustomSpell(
    Unit* target, uint32 spellId, int32 const* bp0, int32 const* bp1, int32 const* bp2, bool triggered, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
...
SpellCastResult Unit::CastCustomSpell(
    uint32 spellId, SpellValueMod mod, int32 value, Unit* target, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    CustomSpellValues values;
    values.AddSpellMod(mod, value);
...
SpellCastResult Unit::CastCustomSpell(
    uint32 spellId, CustomSpellValues const& value, Unit* victim, TriggerCastFlags triggerFlags, Item* castItem, AuraEffect const* triggeredByAura, ObjectGuid originalCaster)
{
    SpellInfo const* spellInfo = sSpellMgr->GetSpellInfo(spellId);
    if (!spellInfo)
    {
...
void Unit::CalculateMeleeDamage(Unit* victim, CalcDamageInfo* damageInfo, WeaponAttackType attackType, const bool sittingVictim)
{
    ...
    if (immunedMask & ((1 << 0) | (1 << 1)))
    {
        if (hasNonPhysicalSchoolMask || immunedMask == ((1 << 0) | (1 << 1)))
        {
            ...
            damageInfo->procEx |= PROC_HIT_IMMUNE;
            return;
        }
    }
    ...
    case MELEE_HIT_EVADE:
        ...
        damageInfo->procEx |= PROC_HIT_EVADE;
    ...
    case MELEE_HIT_MISS:
        ...
        damageInfo->procEx |= PROC_HIT_MISS;
    ...
    case MELEE_HIT_CRIT:
    {
        ...
        damageInfo->procEx |= PROC_HIT_CRITICAL;
    ...
    case MELEE_HIT_PARRY:
        ...
        damageInfo->procEx |= PROC_HIT_PARRY;
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
        {
    ...
    case MELEE_HIT_DODGE:
        ...
        damageInfo->procEx |= PROC_HIT_DODGE;
        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
        {
            damageInfo->cleanDamage += damageInfo->damages[i].damage;
            damageInfo->damages[i].damage = 0;
    ...
    case MELEE_HIT_BLOCK:
        ...
        damageInfo->procEx |= PROC_HIT_BLOCK;

        // 30% damage blocked, double blocked amount if block is critical
        uint32 blockPct = damageInfo->target->isBlockCritical() ? damageInfo->target->GetBlockPercent() * 2 : damageInfo->target->GetBlockPercent();

        for (uint8 i = 0; i < MAX_ITEM_PROTO_DAMAGES; ++i)
        {
            damageInfo->cleanDamage += damageInfo->damages[i].damage;
            damageInfo->blocked_amount += CalculatePct(damageInfo->damages[i].damage, blockPct);
        }

        // TODO is this needed if we use pct???? can we fully block?
        // // full block
    ...
    case MELEE_HIT_GLANCING:
    {
        ...
        if (leveldif < 0)
        {
            LOG_DEBUG("entities.unit", "Unit::CalculateMeleeDamage: (Player) %s attacked %s. Glancing should never happen against lower level target", GetGUID().ToString().c_str(),
                victim->GetGUID().ToString().c_str());
            break;
        }
        if (leveldif == 0)
            leveldif = 1;
...
/*static*/ uint32 Unit::CalcArmorReducedDamage(Unit const* attacker, Unit const* victim, const uint32 damage, SpellInfo const* spellInfo, uint8 attackerLevel, WeaponAttackType /*attackType*/)
{
    ...
    if (attacker)
    {
        ...
        int32 armorBypassPct = victim->GetTotalAuraModifier(SPELL_AURA_BYPASS_ARMOR_FOR_CASTER, [&](AuraEffect const* aurEff) { return aurEff->GetCasterGUID() == attacker->GetGUID(); });
...
/*static*/ uint32 Unit::CalcSpellResistedDamage(Unit const* attacker, Unit* victim, uint32 damage, SpellSchoolMask schoolMask, SpellInfo const* spellInfo)
{
    ...
    if (damageResisted > 0.0f && attacker) // if any damage was resisted
    {
...
/*static*/ float Unit::GetEffectiveResistChance(Unit const* attacker, SpellSchoolMask schoolMask, Unit const* victim, SpellInfo const* spellInfo)
{
    ...
    // Max +3
    static uint32 const BOSS_LEVEL = 85 + 3;
...
void Unit::CalcAbsorbResist(DamageInfo& damageInfo, bool Splited)
{
    ...
    for (AuraEffectList::iterator itr = vSchoolAbsorbCopy.begin(); (itr != vSchoolAbsorbCopy.end()) && (damageInfo.GetDamage() > 0); ++itr)
    {
        ...
        if (absorbAurEff->GetAmount() >= 0)
        {
            ...
            if (absorbAurEff->GetAmount() <= 0)
                absorbAurEff->GetBase()->Remove(AuraRemoveMode::ByEnemySpell);
        }
    }
    ...
    if (existExpired)
    {
        for (AuraEffectList::const_iterator i = vHealAbsorb.begin(); i != vHealAbsorb.end();)
        {
            ...
            if (auraEff->GetAmount() <= 0)
            {
                ...
                auraEff->GetBase()->Remove(AuraRemoveMode::ByEnemySpell);
...
void Unit::AttackerStateUpdate(Unit* victim, WeaponAttackType attType /*= BASE_ATTACK*/, bool extra /*= false*/, bool ignoreCasting /*= false*/)
{
    ...
    if (meleeAttack)
    {
        ...
        Unit::ProcDamageAndSpell(damageInfo.attacker, damageInfo.target, damageInfo.procAttacker, damageInfo.procVictim, damageInfo.procEx, dmgInfo.GetDamage(), damageInfo.attackType, nullptr,
            nullptr, -1, nullptr, &dmgInfo);

        if (IsPlayer())
            LOG_DEBUG("entities.unit", "AttackerStateUpdate: (Player) {} attacked {} for {} dmg, absorbed {}, blocked {}, resisted {}.", GetGUID().ToString(), victim->GetGUID().ToString(),
                dmgInfo.GetDamage(), dmgInfo.GetAbsorb(), dmgInfo.GetBlock(), dmgInfo.GetResist());
        else
            LOG_DEBUG("entities.unit", "AttackerStateUpdate: (NPC) {} attacked {} for {} dmg, absorbed {}, blocked {}, resisted {}.", GetGUID().ToString(), victim->GetGUID().ToString(),
                dmgInfo.GetDamage(), dmgInfo.GetAbsorb(), dmgInfo.GetBlock(), dmgInfo.GetResist());
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const* victim, WeaponAttackType attType) const
{
    ...
    return RollMeleeOutcomeAgainst(victim, attType, crit_chance, miss_chance, dodge_chance, parry_chance, block_chance);
}
...
MeleeHitOutcome Unit::RollMeleeOutcomeAgainst(Unit const* victim, WeaponAttackType attType, int32 crit_chance, int32 miss_chance, int32 dodge_chance, int32 parry_chance, int32 block_chance) const
{
    ...
    bool canParryOrBlock = victim->HasInArc(float(M_PI), this) || victim - HasIgnoreHitDirectionAura();
    ...
    if (canDodge)
    {
        if (victim->IsPlayer() || !(victim->ToCreature()->HasFlagsExtra(CREATURE_FLAG_EXTRA_NO_DODGE)))
        {
    ...
    // 5. GLANCING
    // Max 40% chance to score a glancing blow against mobs of the same or higher level (only players and pets, not for ranged
    // weapons).
    if (glancing)
    {
    ...
    if (getLevelForTarget(victim) >= victim->getLevelForTarget(this) + 4 &&
        ...
        !IsControlledByPlayer() && !(IsCreature() && ToCreature()->GetCreatureTemplate()->flags_extra & CREATURE_FLAG_EXTRA_NO_CRUSH))
    {
...
void Unit::SendMeleeAttackStop(Unit* victim)
{
    ...
    if (victim)
        LOG_DEBUG("entities.unit", "%s %u stopped attacking %s %u", (IsPlayer() ? "Player" : "Creature"), GetGUID().GetCounter(), (victim->IsPlayer() ? "player" : "creature"),
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    bool canUseDefenseWhileChanneling =
        victim->GetCurrentSpell(CURRENT_CHANNELED_SPELL) ? victim->GetCurrentSpell(CURRENT_CHANNELED_SPELL)->GetSpellInfo()->HasAttribute(SPELL_ATTR10_ALLOW_DEFENSE_WHILE_CHANNELING) : false;
    bool canUseDefenseWhileCasting =
        victim->GetCurrentSpell(CURRENT_GENERIC_SPELL) ? victim->GetCurrentSpell(CURRENT_GENERIC_SPELL)->GetSpellInfo()->HasAttribute(SPELL_ATTR10_ALLOW_DEFENSE_WHILE_CASTING) : false;
    ...
    for (AuraEffect const* aurEff : ignore)
    {
        ...
        case MELEE_HIT_PARRY:
            ...
            break;
        default:
            LOG_DEBUG("entities.unit", "Spell %u SPELL_AURA_IGNORE_COMBAT_RESULT has unhandled state %d", aurEff->GetId(), aurEff->GetMiscValue());
            break;
        }
    }
...
SpellMissInfo Unit::MagicSpellHitResult(Unit* victim, SpellInfo const* spellInfo)
{
    ...
    {
        MISS_CHANCE_MULTIPLIER = sWorld->getRate(victim->IsPlayer() ? RATE_MISS_CHANCE_MULTIPLIER_TARGET_PLAYER : RATE_MISS_CHANCE_MULTIPLIER_TARGET_CREATURE);
    }
    ...
    modHitChance *= MISS_CHANCE_MULTIPLIER;
    ...
    if (!victim->HasUnitState(UNIT_STATE_CONTROLLED) && !victim->HasUnitState(UNIT_STATE_STUNNED) && (victim->HasInArc(M_PI, this) || victim->HasIgnoreHitDirectionAura()))
    {
        int32 deflect_chance = victim->GetTotalAuraModifier(SPELL_AURA_DEFLECT_SPELLS) * 100;
        tmp += deflect_chance;
        if (rand < tmp)
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, SpellInfo const* spellInfo, bool canReflect /*= false*/)
{
    ...
    if ((spell->IsPositive() || spell->HasEffect(SPELL_EFFECT_DISPEL)) && !IsHostileTo(victim)) // prevent from affecting enemy by "positive" spell
        return SPELL_MISS_NONE;
    ...
    if (victim->IsCreature() && victim->ToCreature()->IsEvadingAttacks() && !spell->HasAura(SPELL_AURA_CONTROL_VEHICLE) && !spell->HasAttribute(SPELL_ATTR0_CU_IGNORE_EVADE) &&
        !spell->HasAttribute(SPELL_ATTR1_AURA_STAYS_AFTER_COMBAT))
...
SpellMissInfo Unit::SpellHitResult(Unit* victim, Spell const* spell, bool canReflect)
{
    ...
    return SpellHitResult(victim, spellInfo, canReflect);
}
...
float Unit::GetUnitParryChance(WeaponAttackType attType, Unit const* victim) const
{
    ...
    {
        // Allow parries for creatures only if it's not a totem, does have a virtual item equipped and does not have
...
void Unit::_UpdateSpells(uint32 time)
{
    ...
    for (AuraMap::iterator i = m_ownedAuras.begin(); i != m_ownedAuras.end();)
    {
        if (i->second->IsExpired())
            RemoveOwnedAura(i, AuraRemoveMode::Expired);
        else
            ++i;
    }
...
bool Unit::IsNonMeleeSpellCast(bool withDelayed, bool skipChanneled, bool skipAutorepeat, bool isAutoshoot, bool skipInstant) const
{
    ...
    if (!skipChanneled && m_currentSpells[CURRENT_CHANNELED_SPELL] && (m_currentSpells[CURRENT_CHANNELED_SPELL]->getState() != SPELL_STATE_FINISHED))
    {
...
{
    auto itr = std::find_if(m_SummonedCreatures.begin(), m_SummonedCreatures.end(), [entry](auto& p) { return p.second == entry; });
...
AuraApplication* Unit::_CreateAuraApplication(Aura* aura, uint8 effMask)
{
    ...
    if (!IsAlive() && !aurSpellInfo->IsDeathPersistent() && (GetTypeId() != TYPEID_PLAYER || !ToPlayer()->GetSession()->PlayerLoading()))
        return nullptr;
...
void Unit::_UnapplyAura(AuraApplicationMap::iterator& i, AuraRemoveMode removeMode)
{
    ...
    Aura* aura = aurApp->GetBase();
    LOG_DEBUG("spells", "Aura %u now is remove mode %u", aura->GetId(), static_cast<uint32>(removeMode));
    ...
    if (aurApp->GetRemoveMode().HasFlag(AuraRemoveMode::Expired) && GetTypeId() == TYPEID_UNIT && IsTotem())
    {
...
void Unit::_RemoveNoStackAurasDueToAura(Aura* aura)
{
    ...
    for (AuraApplicationMap::iterator i = m_appliedAuras.begin(); i != m_appliedAuras.end(); ++i)
    {
        ...
        RemoveAura(i, AuraRemoveMode::ByDefault);
...
void Unit::RemoveOwnedAura(Aura* aura, AuraRemoveMode removeMode)
{
    ...
    {
        LOG_ERROR("spells", "Unit::RemoveOwnedAura() called with unallowed removeMode AuraRemoveMode::None, spellId %u", aura->GetId());
        return;
    }
...
Aura* Unit::GetOwnedAura(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID, uint8 reqEffMask, Aura* except) const
{
    ...
    {
        if (((itr->second->GetEffectMask() & reqEffMask) == reqEffMask) && (!casterGUID || itr->second->GetCasterGUID() == casterGUID) &&
            (!itemCasterGUID || itr->second->GetCastItemGUID() == itemCasterGUID) && (!except || except != itr->second))
        {
            return itr->second;
        }
    }
...
void Unit::RemoveAuraFromStack(uint32 spellId, ObjectGuid casterGUID, AuraRemoveMode removeMode)
{
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;)
    {
        ...
        if ((aura->GetType() == UNIT_AURA_TYPE) && (!casterGUID || aura->GetCasterGUID() == casterGUID))
        {
    ...
    for (AuraMap::iterator iter = range.first; iter != range.second;)
    {
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            if (aura->GetSpellInfo()->HasAttribute(SPELL_ATTR7_DISPEL_CHARGES))
                aura->ModCharges(-dispelInfo.GetRemovedCharges(), AuraRemoveMode::ByEnemySpell);
            else
                aura->ModStackAmount(-dispelInfo.GetRemovedCharges(), AuraRemoveMode::ByEnemySpell);
        ...
        if (aura->GetCasterGUID() == casterGUID)
        {
            ...
            else
            {
                ...
                if (Aura* newAura = Aura::TryRefreshStackOrCreate(createInfo))
                {
                    ...
                    newAura->SetLoadedState(
                        aura->GetMaxDuration(), int32(dur), stealCharge ? 1 : aura->GetCharges(), 1, recalculateMask, aura->GetCritChance(), aura->CanApplyResilience(), &damage[0]);
                    newAura->ApplyForTargets();
                }
            }

            if (stealCharge)
                aura->ModCharges(-1, AuraRemoveMode::ByEnemySpell);
            else
                aura->ModStackAmount(-1, AuraRemoveMode::ByEnemySpell);
...
void Unit::RemoveAllAuras()
{
    ...
    for (uint32 counter = 0; !m_appliedAuras.empty() || !m_ownedAuras.empty(); counter++)
    {
        ...
        for (aurAppIter = m_appliedAuras.begin(); aurAppIter != m_appliedAuras.end();)
            _UnapplyAura(aurAppIter, AuraRemoveMode::ByDefault);
...
void Unit::DelayOwnedAuras(uint32 spellId, ObjectGuid caster, int32 delaytime)
{
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (!caster || aura->GetCasterGUID() == caster)
        {
            ...
            LOG_DEBUG("spells", "Aura %u partially interrupted on unit %u, new duration: %u ms", aura->GetId(), GetGUID().GetCounter(), aura->GetDuration());
        }
    }
}
...
AuraApplication* Unit::GetAuraApplication(uint32 spellId, ObjectGuid casterGUID, ObjectGuid itemCasterGUID, uint8 reqEffMask, AuraApplication* except) const
{
    ...
    for (; range.first != range.second; ++range.first)
    {
        ...
        if (((aura->GetEffectMask() & reqEffMask) == reqEffMask) && (!casterGUID || aura->GetCasterGUID() == casterGUID) && (!itemCasterGUID || aura->GetCastItemGUID() == itemCasterGUID) &&
            (!except || except != app))
        {
            return app;
        }
...
{
    static const AuraType diseaseAuraTypes[] = {SPELL_AURA_PERIODIC_DAMAGE, SPELL_AURA_PERIODIC_DAMAGE_PERCENT, SPELL_AURA_NONE};
...
float Unit::GetTotalAuraMultiplierByMiscMask(AuraType auraType, uint32 miscMask) const
{
    return GetTotalAuraMultiplier(auraType,
        [miscMask](AuraEffect const* aurEff) -> bool
        {
...
int32 Unit::GetMaxPositiveAuraModifierByMiscMask(AuraType auraType, uint32 miscMask, AuraEffect const* except /*= nullptr*/) const
{
    return GetMaxPositiveAuraModifier(auraType,
        [miscMask, except](AuraEffect const* aurEff) -> bool
        {
...
int32 Unit::GetTotalAuraModifierByMiscValue(AuraType auraType, int32 miscValue) const
{
    return GetTotalAuraModifier(auraType,
        [miscValue](AuraEffect const* aurEff) -> bool
        {
...
void Unit::ApplyResistanceBuffModsMod(SpellSchools school)
{
    ...
    modPos += GetTotalAuraModifier(SPELL_AURA_MOD_RESISTANCE,
        [school](AuraEffect const* aurEff) -> bool
        {
    ...
    modNeg = GetTotalAuraModifier(SPELL_AURA_MOD_RESISTANCE,
        [school](AuraEffect const* aurEff) -> bool
        {
            if ((aurEff->GetMiscValue() & (1 << school)) && aurEff->GetAmount() < 0)
                return true;
...
void Unit::ApplyStatBuffMod(Stats stat)
{
    ...
    modPos += GetTotalAuraModifier(SPELL_AURA_MOD_STAT,
        [stat](AuraEffect const* aurEff) -> bool
        {
    ...
    modNeg += GetTotalAuraModifier(SPELL_AURA_MOD_STAT,
        [stat](AuraEffect const* aurEff) -> bool
        {
            if ((aurEff->GetMiscValue() < 0 || aurEff->GetMiscValue() == stat) && aurEff->GetAmount() < 0)
                return true;
    ...
    factor = GetTotalAuraMultiplier(SPELL_AURA_MOD_PERCENT_STAT,
        [stat](AuraEffect const* aurEff) -> bool
        {
    ...
    factor *= GetTotalAuraMultiplier(SPELL_AURA_MOD_TOTAL_STAT_PERCENTAGE,
        [stat](AuraEffect const* aurEff) -> bool
        {
            if (aurEff->GetMiscValue() == -1 || aurEff->GetMiscValue() == stat)
                return true;
...
/*static*/ void Unit::ProcSkillsAndAuras(Unit* actor, Unit* actionTarget, uint32 typeMaskActor, uint32 typeMaskActionTarget, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell* spell,
...
void Unit::SendAttackStateUpdate(
                    ...
                    if (!selfPlayerOwner->HasFlag(UNIT_FIELD_FLAGS_2, UNIT_FLAG2_IGNORE_REPUTATION))
                    {
                        if (FactionEntry const* targetFactionEntry = sFactionStore.LookupEntry(targetFactionTemplateEntry->Faction))
                        {
                            if (targetFactionEntry->CanHaveReputation())
                            {
                                ...
                                if (targetFactionTemplateEntry->IsContestedGuardFaction() && selfPlayerOwner->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_CONTESTED_PVP))
                                    return REP_HOSTILE;
...
ReputationRank Unit::GetFactionReactionTo(FactionTemplateEntry const* factionTemplateEntry, Unit const* target)
{
    ...
    {
        // check contested flags
        if (factionTemplateEntry->IsContestedGuardFaction() && targetPlayerOwner->HasFlag(PLAYER_FLAGS, PLAYER_FLAGS_CONTESTED_PVP))
            return REP_HOSTILE;
...
void Unit::SetMinion(Minion* minion, bool apply)
{
    if (!minion)
    {
        LOG_ERROR("entities.unit", "Unit::SetMinion: Unit %s tried to reference a non existing minion", GetGUID().ToString().c_str());
        return;
    }
...
void Unit::GetAllMinionsByEntry(std::list<Creature*>& Minions, uint32 entry)
{
    for (Unit::ControlList::iterator itr = m_Controlled.begin(); itr != m_Controlled.end();)
    {
        Unit* unit = *itr;
        ++itr;
        if (unit->GetEntry() == entry && unit->GetTypeId() == TYPEID_UNIT && unit->IsSummon()) // minion, actually
...
void Unit::SetCharm(Unit* charm, bool apply)
{
    if (apply)
    {
        if (IsPlayer())
        {
            ASSERT(AddGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()), "Player %s is trying to charm unit %u, but it already has a charmed unit %s", GetName().c_str(), charm->GetEntry(),
        ...
        ASSERT(charm->AddGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()), "Unit %u is being charmed, but it already has a charmer %s", charm->GetEntry(), charm->GetCharmerGUID().ToString().c_str());
    ...
    else
    {
        ...
        if (IsPlayer())
        {
            ASSERT(RemoveGuidValue(UNIT_FIELD_CHARM, charm->GetGUID()), "Player %s is trying to uncharm unit %u, but it has another charmed unit %s", GetName().c_str(), charm->GetEntry(),
                GetCharmedGUID().ToString().c_str());
            m_charmed = nullptr;
        }

        ASSERT(charm->RemoveGuidValue(UNIT_FIELD_CHARMEDBY, GetGUID()), "Unit %u is being uncharmed, but it has another charmer %s", charm->GetEntry(), charm->GetCharmerGUID().ToString().c_str());
        ...
        if (charm->IsPlayer() || !charm->ToCreature()->HasUnitTypeMask(UNIT_MASK_MINION) || charm->GetOwnerOrCreatorGUID() != GetGUID())
        {
        ...
        if (Unit* magnet = (*itr)->GetBase()->GetCaster())
            if (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK && IsValidAttackTarget(magnet, spellInfo))
            {
                ...
                if (spellInfo->Speed > 0.0f)
                {
                    ...
                    (*itr)->GetBase()->DropChargeDelayed(delay, AuraRemoveMode::Expired);
                }
                else
                    (*itr)->GetBase()->DropCharge(AuraRemoveMode::Expired);
    ...
    for (AuraEffectList::const_iterator i = hitTriggerAuras.begin(); i != hitTriggerAuras.end(); ++i)
    {
        if (Unit* magnet = (*i)->GetBase()->GetCaster())
            if (IsValidAttackTarget(magnet, spellInfo) && magnet->IsWithinLOSInMap(this) &&
                (!spellInfo || (spellInfo->CheckExplicitTarget(this, magnet) == SPELL_CAST_OK && spellInfo->CheckTarget(this, magnet, false) == SPELL_CAST_OK)))
                if (roll_chance_i((*i)->GetAmount()))
                {
                    (*i)->GetBase()->DropCharge(AuraRemoveMode::Expired);
                    return magnet;
                }
    }
    return victim;
...
float Unit::SpellDamagePctDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype) const
{
    ...
    DoneTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_DONE_VERSUS_AURASTATE,
        [victim](AuraEffect const* aurEff) -> bool
        {
    ...
    case SPELLFAMILY_MAGE:
        ...
        {
            if (victim->HasAuraState(AURA_STATE_FROZEN, spellProto, this) || GetAuraEffect(SPELL_AURA_ABILITY_IGNORE_AURASTATE, SPELLFAMILY_MAGE, 0, 0, 0x0000000A, GetGUID()))
                DoneTotalMod *= 2.0f;
        }
...
uint32 Unit::SpellDamageBonusTaken(Unit* caster, SpellInfo const* spellProto, uint32 pdamage, DamageEffectType damagetype) const
{
    ...
    {
        TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT,
            [mechanicMask, spellProto](AuraEffect const* aurEff)
            {
                if (spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                    return false;
    ...
    if (!spellProto->HasAttribute(SPELL_ATTR4_FIXED_DAMAGE))
    {
        ...
        if (caster)
        {
            TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_FROM_CASTER,
                [caster, spellProto](AuraEffect const* aurEff)
                {
                    if (spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                        return false;
    ...
    case SPELL_DAMAGE_CLASS_MELEE:
    {
        ...
        if (caster)
        {
            ...
            case SPELLFAMILY_DRUID:
                ...
                if (spellInfo->SpellFamilyFlags[0] & 0x00800000 && spellInfo->SpellIconID == 1680 && HasAuraState(AURA_STATE_BLEEDING))
                {
...
/*static*/ uint32 Unit::SpellCriticalDamageBonus(Unit const* caster, SpellInfo const* spellProto, uint32 damage, Unit const* victim)
{
    ...
    if (SpellBonusEntry const* bonus = sSpellMgr->GetSpellBonusData(spellProto->Id))
    {
        ...
        if (damagetype == DOT)
        {
            ...
            if (bonus->ap_dot_bonus > 0)
                DoneTotal += int32(
                    bonus->ap_dot_bonus * stack * GetTotalAttackPowerValue((spellProto->IsRangedWeaponSpell() && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
        else
        {
            ...
            if (bonus->ap_bonus > 0)
                DoneTotal +=
                    int32(bonus->ap_bonus * stack * GetTotalAttackPowerValue((spellProto->IsRangedWeaponSpell() && spellProto->DmgClass != SPELL_DAMAGE_CLASS_MELEE) ? RANGED_ATTACK : BASE_ATTACK));
        }
    }
    else
    {
...
float Unit::SpellPctHealingModsDone(Unit* victim, SpellInfo const* spellProto, DamageEffectType damagetype)
{
    ...
    {
        if (!overrideClassScripts->IsAffectedOnSpell(spellProto))
            continue;
    ...
    case SPELLFAMILY_SHAMAN:
    {
        ...
        if (AuraEffect const* blessing = GetDummyAuraEffect(SPELLFAMILY_SHAMAN, 2012, EFFECT_0))
            if (blessing->IsAffectedOnSpell(spellProto) && victim->HasAura(974, GetGUID()))
...
int32 Unit::SpellBaseHealingBonusDone(SpellSchoolMask schoolMask, bool withSpellPowerPctMod /*= false*/) const
{
    ...
    int32 advertisedBenefit = GetTotalAuraModifier(SPELL_AURA_MOD_HEALING_DONE,
        [schoolMask](AuraEffect const* aurEff) -> bool
        {
    ...
    SpellSchoolMask damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    if (!spellProto)
    {
        ...
        AddPct(DoneTotalMod, victim->GetTotalAuraMultiplier(SPELL_AURA_MOD_MELEE_DAMAGE_FROM_CASTER, [this](AuraEffect const* effect) -> bool { return effect->GetCasterGUID() == GetGUID(); }));
    }
...
uint32 Unit::MeleeDamageBonusTaken(Unit* attacker, uint32 pdamage, WeaponAttackType attType, SpellInfo const* spellProto /*= nullptr*/, SpellSchoolMask damageSchoolMask /*= SPELL_SCHOOL_MASK_NORMAL*/)
{
    ...
    // ..taken
    TakenFlatBenefit += GetTotalAuraModifier(SPELL_AURA_MOD_DAMAGE_TAKEN,
        [meleeDamageSchoolMask, spellProto](AuraEffect const* aurEff)
        {
    ...
    TakenFlatBenefit += GetTotalAuraModifier(modDamageTaken,
        [spellProto](AuraEffect const* aurEff)
        {
            if (spellProto && spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                return false;
    ...
    if (spellProto)
    {
        // From caster spells
        TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_FROM_CASTER,
            [attacker, spellProto](AuraEffect const* aurEff) -> bool
            {
                if (spellProto && spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                    return false;
        ...
        {
            TakenTotalMod *= GetTotalAuraMultiplier(SPELL_AURA_MOD_MECHANIC_DAMAGE_TAKEN_PERCENT,
                [mechanicMask, spellProto](AuraEffect const* aurEff) -> bool
                {
                    if (spellProto && spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                        return false;
    ...
    TakenTotalMod *= GetTotalAuraMultiplier(modDamageTakenPct,
        [spellProto](AuraEffect const* aurEff)
        {
            if (spellProto && spellProto->HasAttribute(SPELL_ATTR10_IGNORE_POSITIVE_DAMAGE_TAKEN_MODS) && aurEff->GetAmount() > 0)
                return false;
...
MountCapabilityEntry const* Unit::GetMountCapability(uint32 mountType) const
{
    ...
    for (uint32 i = MAX_MOUNT_CAPABILITIES; i > 0; --i)
    {
        ...
        if (mountCapability->ReqMapID != -1 && int32(GetMapId()) != mountCapability->ReqMapID && GetMap()->GetEntry()->ParentMapID != mountCapability->ReqMapID)
            continue;
...
bool Unit::IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell) const { return _IsValidAttackTarget(target, bySpell); }
...
bool Unit::_IsValidAttackTarget(Unit const* target, SpellInfo const* bySpell, WorldObject const* obj) const
{
    ...
    if ((!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_IGNORE_PHASE_SHIFT)) &&
        (obj ? !obj->CanSeeOrDetect(target, bySpell && bySpell->IsAffectingArea())
             : !CanSeeOrDetect(target, (bySpell && bySpell->IsAffectingArea()) || (target->IsPlayer() && target->HasStealthAura() && target->IsInCombat() && IsInCombatWith(target)))))
        return false;
    ...
    if (repThisToTarget > REP_NEUTRAL || (repTargetToThis = target->GetReactionTo(this)) > REP_NEUTRAL)
        return false;

    // Not all neutral creatures can be attacked (even some unfriendly faction does not react aggresive to you, like Sporaggar)
    if (repThisToTarget == REP_NEUTRAL && repTargetToThis <= REP_NEUTRAL)
    {
        Player* owner = GetAffectingPlayer();
        Unit const* const thisUnit = owner ? owner : this;
        if (!(target->IsPlayer() && thisUnit->IsPlayer()) && !(target->IsCreature() && thisUnit->IsCreature()))
        {
...
bool Unit::_IsValidAssistTarget(Unit const* target, SpellInfo const* bySpell) const
{
    ...
    else if (HasUnitFlag(UNIT_FLAG_PLAYER_CONTROLLED) && (!bySpell || !bySpell->HasAttribute(SPELL_ATTR6_CAN_ASSIST_IMMUNE_PC)) && !target->IsPvP())
    {
...
void Unit::UpdateSpeed(UnitMoveType mtype)
{
    ...
    case MOVE_FLIGHT:
    {
        ...
        if (IsCreature())
        {
            ...
            speed *=
                movementInfo.HasRunSpeedOverriden ? (movementInfo.RunSpeed / baseMoveSpeed[MOVE_RUN]) : ToCreature()->GetCreatureTemplate()->speed_run; // at this point, MOVE_WALK is never reached
        }
...
void Unit::SetSpeedRate(UnitMoveType mtype, float rate)
{
    ...
    // explaination of (1):
    // If the player is not in the world yet, it won't reply to the packets requiring an ack. And once the player is in the world,
    // next time a movement packet which requires an ack is sent to the client (change of speed for example), the client is kicked
    // from the server on the ground that it should have replied to the first packet first. That line is a hacky fix in the sense
    // that it doesn't work like that in retail since buffs are applied only after the player has been initialized in the world.
    // cf description of PR #18771
}
...
void Unit::FollowTarget(Unit* target)
{
    if (!target)
    {
        LOG_ERROR("entities.unit", "Unit::FollowTarget: Unit (%s) tried to follow a non-existant target.", GetGUID().ToString().c_str());
        return;
    }
...
void Unit::AtEnterCombat()
{
    if (Spell* spell = m_currentSpells[CURRENT_GENERIC_SPELL])
        if (spell->getState() == SPELL_STATE_PREPARING && spell->m_spellInfo->HasAttribute(SPELL_ATTR0_NOT_IN_COMBAT_ONLY_PEACEFUL) &&
...
void Unit::AtTargetAttacked(Unit* target, bool canInitialAggro)
{
    ...
    // Patch 3.0.8: All player spells which cause a creature to become aggressive to you will now also immediately cause the
    // creature to be tapped.
...
{
    ASSERT(!IsPet()); // player pets do not use UNIT_FLAG_PET_IN_COMBAT for this purpose - but player pets should also never have
                      // minions of their own to call this
...
int32 Unit::ModSpellDuration(SpellInfo const* spellProto, Unit const* target, int32 duration, bool positive, uint32 effectMask)
{
    ...
    else
    {
        ...
        {
            if (spellProto->SpellFamilyName == SPELLFAMILY_POTION &&
                (sSpellMgr->IsSpellMemberOfSpellGroup(spellProto->Id, SPELL_GROUP_ELIXIR_BATTLE) || sSpellMgr->IsSpellMemberOfSpellGroup(spellProto->Id, SPELL_GROUP_ELIXIR_GUARDIAN)))
            {
...
void Unit::ModSpellCastTime(SpellInfo const* spellInfo, int32& castTime, Spell* spell)
{
    ...
    if (!(spellInfo->HasAttribute(SPELL_ATTR0_IS_ABILITY) || spellInfo->HasAttribute(SPELL_ATTR0_IS_TRADESKILL) || spellInfo->HasAttribute(SPELL_ATTR3_NO_DONE_BONUS)) &&
        ((IsPlayer() && spellInfo->SpellFamilyName) || GetTypeId() == TYPEID_UNIT))
...
void Unit::UpdateDamageDoneMods(WeaponAttackType attackType)
{
    ...
    float amount = GetTotalAuraModifier(SPELL_AURA_MOD_DAMAGE_DONE,
        [&](AuraEffect const* aurEff) -> bool
        {
            if (!(aurEff->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
                return false;
...
void Unit::UpdateDamagePctDoneMods(WeaponAttackType attackType)
{
    ...
    factor *= GetTotalAuraMultiplier(SPELL_AURA_MOD_DAMAGE_PERCENT_DONE,
        [attackType, this](AuraEffect const* aurEff) -> bool
        {
            if (!(aurEff->GetMiscValue() & SPELL_SCHOOL_MASK_NORMAL))
                return false;
...
{
    return !HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_FLEEING | UNIT_STATE_IN_FLIGHT | UNIT_STATE_ROOT | UNIT_STATE_STUNNED | UNIT_STATE_DISTRACTED) && GetOwnerOrCreatorGUID().IsEmpty();
}
...
void Unit::RemoveFromWorld()
{
    ...
    if (IsInWorld())
    {
        ...
        if (Unit* owner = GetOwner())
        {
            ...
            {
                LOG_FATAL("entities.unit", "Unit %u is in controlled list of %u when removed from world", GetEntry(), owner->GetEntry());
                ABORT();
            }
        }
...
void Unit::CleanupBeforeRemoveFromMap(bool finalCleanup)
{
    ...
    m_Events.KillAllEvents(false); // non-delatable (currently cast spells) will not deleted now but it will deleted at call in
        ...
        if (IsPlayer())
        {
            if (Unit* charmer = GetCharmer())
            {
                ...
                if (Creature* creatureCharmer = charmer->ToCreature())
                {
                    ...
                    LOG_ERROR("misc", "Attempt to assign charm AI to player %s who is charmed by non-creature %s.", GetGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str());
            }
...
CharmInfo::CharmInfo(Unit* unit)
    : _unit(unit), _CommandState(COMMAND_FOLLOW), _petnumber(0), _oldReactState(REACT_PASSIVE), _isCommandAttack(false), _isCommandFollow(false), _isAtStay(false), _isFollowing(false),
      _isReturning(false), _stayX(0.0f), _stayY(0.0f), _stayZ(0.0f)
{
    ...
    if (typeMask & MELEE_BASED_TRIGGER_MASK && procTarget)
    {
        ...
        if (hitMask & (PROC_HIT_CRITICAL | PROC_HIT_PARRY | PROC_HIT_DODGE | PROC_HIT_BLOCK))
        {
            ...
            else // For attacker
            {
                ...
                if ((hitMask & PROC_HIT_DODGE) && IsPlayer() && getClass() == CLASS_WARRIOR)
                {
...
void Unit::TriggerAurasProcOnEvent(
    Unit* actionTarget, uint32 typeMaskActor, uint32 typeMaskActionTarget, uint32 spellTypeMask, uint32 spellPhaseMask, uint32 hitMask, Spell* spell, DamageInfo* damageInfo, HealInfo* healInfo)
{
...
void Unit::RecalculateObjectScale()
{
    ...
    float scaleMin = IsPlayer() ? 0.1 : 0.01;
...
void Unit::ApplyAttackTimePercentMod(WeaponAttackType att, float val, bool apply)
{
    ...
    float hasteAmount = IsPlayer() ? GetFloatValue(PLAYER_FIELD_MOD_HASTE) : 0.f;
    float rangedHasteAmount = IsPlayer() ? GetFloatValue(PLAYER_FIELD_MOD_RANGED_HASTE) : 0.f;
...
/*static*/ void Unit::Kill(Unit* attacker, Unit* victim, bool durabilityLoss /*= true*/)
{
    ...
    if (player)
    {
        ...
        if (pet && pet->IsAlive() && pet->isControlled())
        {
            if (pet->IsAIEnabled)
                ...
                LOG_ERROR("entities.unit", "Pet doesn't have any AI in Unit::Kill(). %s", pet->GetGUID().ToString().c_str());
        }
    }
    ...
    if (Player* plrVictim = victim->ToPlayer())
    {
        ...
        if ((durabilityLoss && !player && !victim->ToPlayer()->InBattleground()) || (player && sWorld->getBoolConfig(CONFIG_DURABILITY_LOSS_IN_PVP)))
        {
            ...
            LOG_DEBUG("entities.unit", "We are dead, losing %u percent durability", loss);
...
bool Unit::SetCharmedBy(Unit* charmer, CharmType type, AuraApplication const* aurApp)
{
    ...
    LOG_DEBUG(
        "entities.unit", "SetCharmedBy: charmer %u (GUID %u), charmed %u (GUID %u), type %u.", charmer->GetEntry(), charmer->GetGUID().GetCounter(), GetEntry(), GetGUID().GetCounter(), uint32(type));

    if (this == charmer)
    {
        LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Unit %u (GUID %u) is trying to charm itself!", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    {
        LOG_FATAL("entities.unit", "Unit::SetCharmedBy: Player on transport is trying to charm %u (GUID %u)", GetEntry(), GetGUID().GetCounter());
        return false;
    }
    ...
    if (GetCharmerGUID())
    {
        LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) has already been charmed but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(),
            charmer->GetGUID().GetCounter());
        return false;
    }
    ...
    if (!IsInWorld())
    {
        LOG_FATAL("entities.unit", "Unit::SetCharmedBy: %u (GUID %u) is not in world but %u (GUID %u) is trying to charm it!", GetEntry(), GetGUID().GetCounter(), charmer->GetEntry(),
            charmer->GetGUID().GetCounter());
        return false;
    }
            ...
            if (GetTypeId() == TYPEID_UNIT && charmer->getClass() == CLASS_WARLOCK)
            {
                ...
                if (cinfo && cinfo->type == CREATURE_TYPE_DEMON)
                {
                    ...
                    if (GetCharmInfo())
                        ...
                        LOG_ERROR("entities.unit", "Aura::HandleModCharm: %s has a charm aura but no charm info!", GetGUID().ToString().c_str());
                }
            }
            break;
        case CHARM_TYPE_CONVERT:
...
void Unit::SendPlaySpellVisual(uint32 spellVisualId, Unit const* target /*= nullptr*/, Optional<Position> targetPosition /*= {}*/, float travelSpeed /*= 0.f*/, uint16 missReason /*= 0*/,
    uint16 reflectStatus /*= 0*/, bool speedAsTime /*= false*/) const
{
...
void Unit::OnPhaseChange()
{
    ...
    // Inform player groups that a member's phase has changed so the next PartyMemberState packet will update the phasing
    // accordingly
    ...
    for (SpellClickInfoContainer::const_iterator itr = clickPair.first; itr != clickPair.second; ++itr)
    {
        ...
        if (seatId > -1)
        {
            ...
            else // This can happen during Player::_LoadAuras
            {
                ...
                createInfo.SetCaster(clicker).SetBaseAmount(bp).SetCasterGUID(origCasterGUID);
        ...
        else
        {
            ...
            {
                AuraCreateInfo createInfo(spellEntry, MAX_EFFECT_MASK, this);
                createInfo.SetCaster(clicker).SetCasterGUID(origCasterGUID);
...
void Unit::_EnterVehicle(Vehicle* vehicle, int8 seatId, AuraApplication const* aurApp)
{
    ...
    if (m_vehicle)
    {
        ...
        {
            LOG_DEBUG("entities.vehicle", "EnterVehicle: %u exit %u and enter %u.", GetEntry(), m_vehicle->GetBase()->GetEntry(), vehicle->GetBase()->GetEntry());
            ExitVehicle();
        }
...
void Unit::_ExitVehicle(Position const* exitPosition)
{
    ...
    if (!vehicle)
    {
        LOG_ERROR("entities.vehicle", "RemovePassenger() couldn't remove current unit from vehicle.");
        return;
    }
    ...
    if (GetTypeId() == TYPEID_UNIT && !CanFly() &&
        height > GetMap()->GetWaterOrGroundLevel(GetPhaseShift(), pos.GetPositionX(), pos.GetPositionY(), pos.GetPositionZ() + vehicle->GetBase()->GetCollisionHeight(), &height))
        init.SetFall();
...
{
    // Mirror client behavior, if this method returns false then client will not use swimming animation and for players will apply
    // gravity as if there was no water
...
void Unit::WriteMovementInfo(WorldPacket& data, Movement::ExtraMovementStatusElement* extras /*= nullptr*/, uint32* movementCounter /*= nullptr*/)
{
    ...
    if (!sequence)
    {
        LOG_ERROR("network", "Unit::WriteMovementInfo: No movement sequence found for opcode %s", GetOpcodeNameForLogging(static_cast<OpcodeClient>(data.GetOpcode())).c_str());
        return;
    }
...
bool Unit::UpdatePosition(float x, float y, float z, float orientation, bool teleport)
{
    ...
    {
        LOG_DEBUG("entities.unit", "Unit::UpdatePosition(%f, %f, %f) .. bad coordinates!", x, y, z);
        return false;
    }
    ...
    // G3D::fuzzyEq won't help here, in some cases magnitudes differ by a little more than G3D::eps, but should be considered
    // equal
    bool const relocated = (teleport || std::fabs(GetPositionX() - x) > 0.001f || std::fabs(GetPositionY() - y) > 0.001f || std::fabs(GetPositionZ() - z) > 0.001f);
...
void Unit::CheckPendingMovementAcks()
{
    ...
    for (std::pair<MovementChangeType const, PlayerMovementPendingChange> const& pendingChange : m_pendingMovementChanges)
    {
        ...
        {
            /*
            when players are teleported from one corner of a map to an other (example: from Dragonblight to the entrance of
            Naxxramas, both in the same map: Northend), is it done through what is called a 'near' teleport. A near teleport
            always involve teleporting a player from one point to an other in the same map, even if the distance is huge. When
            that distance is big enough, a loading screen appears on the client side. During that time, the client loads the
            surrounding zone of the new location (and everything it contains). The problem is that, as long as the client hasn't
            finished loading the new zone, it will NOT ack the near teleport. So if the server sends a near teleport order at a
            certain time and the client takes 20s to load the new zone (let's imagine a very slow computer), even with zero
            latency, the server will receive an ack from the client only after 20s.

            For this reason and because the current implementation is simple (you dear reader, feel free to improve it if you
            can), we will just ignore checking for near teleport acks (for now. @todo).
            */
            ...
            LOG_INFO("cheat",
                "Unit::CheckPendingMovementAcks: Player GUID: %s took too long to acknowledge a movement change. He was "
                "therefore kicked.",
                controller->GetBasePlayer()->GetGUID().ToString().c_str());
            break;
        }
    }
}
...
void Unit::PurgeAndApplyPendingMovementChanges(bool informObservers /* = true */)
{
    for (std::pair<MovementChangeType const, PlayerMovementPendingChange> const& pendingChange : m_pendingMovementChanges)
    {
        ...
        switch (changeType)
        {
        case MovementChangeType::SPEED_CHANGE_WALK:
            moveType = MOVE_WALK;
            break;
        case MovementChangeType::SPEED_CHANGE_RUN:
            moveType = MOVE_RUN;
            break;
        case MovementChangeType::SPEED_CHANGE_RUN_BACK:
            moveType = MOVE_RUN_BACK;
            break;
        case MovementChangeType::SPEED_CHANGE_SWIM:
            moveType = MOVE_SWIM;
            break;
        case MovementChangeType::SPEED_CHANGE_SWIM_BACK:
            moveType = MOVE_SWIM_BACK;
            break;
        case MovementChangeType::RATE_CHANGE_TURN:
            moveType = MOVE_TURN_RATE;
            break;
        case MovementChangeType::SPEED_CHANGE_FLIGHT_SPEED:
            moveType = MOVE_FLIGHT;
            break;
        case MovementChangeType::SPEED_CHANGE_FLIGHT_BACK_SPEED:
            moveType = MOVE_FLIGHT_BACK;
            break;
        case MovementChangeType::RATE_CHANGE_PITCH:
...
void Unit::OutDebugInfo() const
{
    ...
    LOG_DEBUG("entities.unit", "%s name %s", GetGUID().ToString().c_str(), GetName().c_str());
    LOG_DEBUG("entities.unit", "Owner %s, Minion %s, Charmer %s, Charmed %s", GetOwnerOrCreatorGUID().ToString().c_str(), GetMinionGUID().ToString().c_str(), GetCharmerGUID().ToString().c_str(),
        GetCharmedGUID().ToString().c_str());
    LOG_DEBUG("entities.unit", "In world %u, unit type mask %u", (uint32)(IsInWorld() ? 1 : 0), m_unitTypeMask);
    if (IsInWorld())
        LOG_DEBUG("entities.unit", "Mapid %u", GetMapId());
    ...
    LOG_DEBUG("entities.unit", "%s", o.str().c_str());
    o.str("");
...
void Unit::BuildValuesUpdate(uint8 updateType, ByteBuffer* data, Player* target) const
{
    ...
    for (uint16 index = 0; index < valCount; ++index)
    {
            ...
            (index == UNIT_FIELD_AURASTATE && HasFlag(UNIT_FIELD_AURASTATE, PER_CASTER_AURA_STATE_MASK)))
        {
            ...
            else if ((index >= UNIT_FIELD_NEGSTAT0 && index <= UNIT_FIELD_NEGSTAT4) || (index >= UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE && index <= (UNIT_FIELD_RESISTANCEBUFFMODSPOSITIVE + 6)) ||
                     (index >= UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE && index <= (UNIT_FIELD_RESISTANCEBUFFMODSNEGATIVE + 6)) || (index >= UNIT_FIELD_POSSTAT0 && index <= UNIT_FIELD_POSSTAT4))
            {
                ...
                if (IsControlledByPlayer() && target != this && sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_GROUP) && IsInRaidWith(target))
                {
                    ...
                    if (ft1 && ft2 && !ft1->IsFriendlyTo(ft2))
                    {
                        if (index == UNIT_FIELD_BYTES_2)
                            ...
                            fieldBuffer << (m_uint32Values[UNIT_FIELD_BYTES_2] &
                                            ((UNIT_BYTE2_FLAG_SANCTUARY /*| UNIT_BYTE2_FLAG_AURAS | UNIT_BYTE2_FLAG_UNK5*/) << 8)); // this flag is at uint8 offset 1 !!
                        else
                            // pretend that all other HOSTILE players have own faction, to allow follow, heal, rezz (trade wont
                            // work)
...
void Unit::Whisper(uint32 textId, Player* target, bool isBossWhisper /*= false*/)
{
    ...
    if (!bct)
    {
        LOG_ERROR("entities.unit", "WorldObject::Whisper: `broadcast_text` was not %u found", textId);
        return;
    }
...
void Unit::ProcessPendingSpellCastRequest()
{
    ...
    if (caster->IsPlayer() && !caster->ToPlayer()->HasActiveSpell(spellInfo->Id) && !spellInfo->IsRaidMarker() &&
    ...
    if (spellInfo->IsPositive())
    {
        if (Unit* target = targets.GetUnitTarget())
        {
            ...
            // if rank not found then function return NULL but in explicit cast case original spell can be cast and later failed
...
void Unit::GetAreaTriggerListWithSpellIDInRange(std::list<AreaTrigger*>& list, uint32 spellid, float fMaxSearchRange) const
{
    ...
    {
        list.remove_if(
            [spellid](AreaTrigger* p_AreaTrigger) -> bool
            {
...
bool Unit::CanNotReachTarget() const { return m_cannotReachTarget; }
