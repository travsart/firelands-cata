...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recvData)
{
    ...
    if (!creature)
    {
        LOG_DEBUG("network", "WORLD: HandlePetitionBuyOpcode - %s not found or you can't interact with him.", guidNPC.ToString().c_str());
        return;
    }
    ...
    if (Petition const* petition = sPetitionMgr->GetPetitionByOwnerWithType(_player->GetGUID(), type))
    {
        ...
        LOG_DEBUG("network", "Invalid petition GUID: %u", petition->petitionGuid.GetCounter());
    }
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recvData)
{
    ...
    if (!petition)
    {
        LOG_DEBUG("entities.player.items", "Petition %s is not found for player %u %s", petitionGuid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter(), GetPlayer()->GetName().c_str());
        return;
    }
...
{
    LOG_DEBUG("network", "Received opcode MSG_PETITION_RENAME");
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recvData)
{
    ...
    if (!petition)
    {
        LOG_ERROR("network", "Petition %s is not found for player %u %s", petitionGuid.ToString().c_str(), GetPlayer()->GetGUID().GetCounter(), GetPlayer()->GetName().c_str());
        return;
    }
    ...
    LOG_DEBUG("network", "PETITION SIGN: %s by player: %s (GUID: %u Account: %u)", petitionGuid.ToString().c_str(), _player->GetName().c_str(), playerGuid.GetCounter(), GetAccountId());
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket& recvData)
{
    ...
    ObjectGuid petitionguid;
    recvData >> petitionguid;                              // petition guid
    LOG_DEBUG("network", "Petition %s declined by %u", petitionguid.ToString().c_str(), _player->GetGUID().GetCounter());
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network", "OFFER PETITION: type %u, %s, to %s", static_cast<uint32>(type), petitionGuid.ToString().c_str(), offererGuid.ToString().c_str());
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recvData)
{
    ...
    LOG_DEBUG("network", "Petition %s turned in by %u", petitionGuid.ToString().c_str(), _player->GetGUID().GetCounter());

    Petition const* petition = sPetitionMgr->GetPetition(petitionGuid);
    if (!petition)
    {
        LOG_ERROR("entities.player.cheat", "Player %s (guid: %u) tried to turn in petition (%s) that is not present in the database", _player->GetName().c_str(), _player->GetGUID().GetCounter(), petitionGuid.ToString().c_str());
        return;
    }
    ...
    else
    {
        ...
        for (Signature const& signature : signatures)
        {
            LOG_DEBUG("network", "PetitionsHandler: Adding arena team (guid: %u) member %s", arenaTeam->GetId(), signature.second.ToString().c_str());
    ...
    // created
    LOG_DEBUG("network", "Player %s (%s) turning in petition %s", _player->GetName().c_str(), _player->GetGUID().ToString().c_str(), petitionGuid.ToString().c_str());
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    if (!creature)
    {
        LOG_DEBUG("network", "WORLD: HandlePetitionShowListOpcode - %s not found or you can't interact with him.", guid.ToString().c_str());
        return;
    }
