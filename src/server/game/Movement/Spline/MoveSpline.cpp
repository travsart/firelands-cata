...
void MoveSpline::init_spline(MoveSplineInitArgs const& args)
{
    ...
    {
        LOG_DEBUG("misc", "MoveSpline::init_spline: zero length spline, wrong input data?");
...
#define CHECK(exp) \
    ...
    {\
        if (unit)\
            LOG_ERROR("misc.movesplineinitargs", "MoveSplineInitArgs::Validate: expression '%s' failed for GUID: %u Entry: %u", #exp, unit->IsPlayer() ? unit->GetGUID().GetCounter() : unit->ToCreature()->GetSpawnId(), unit->GetEntry());\
        else\
...
bool MoveSplineInitArgs::_checkPathBounds() const
{
    if (!(flags & MoveSplineFlag::Catmullrom) && path.size() > 2)
    {
        ...
        for (uint32 i = 1; i < path.size()-1; ++i)
        {
            ...
            {
                LOG_ERROR("misc", "MoveSplineInitArgs::_checkPathBounds check failed");
                return false;
            }
        }
    }
