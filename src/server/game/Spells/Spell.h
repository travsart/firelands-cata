...
struct FC_GAME_API SpellDestination
{
    SpellDestination();
...
class FC_GAME_API SpellCastTargets
{
  public:
    SpellCastTargets();
...
class FC_GAME_API Spell
{
  ...
  public:
    ...
    void SearchChainTargets(std::list<WorldObject*>& targets, uint32 chainTargets, WorldObject* target, SpellTargetObjectTypes objectType, SpellTargetCheckTypes selectType,
        ConditionContainer* condList, bool isChainHeal);
    ...
    static void WriteCastResultInfo(
    ...
    static void SendCastResult(Player* caster, SpellInfo const* spellInfo, uint8 castCount, SpellCastResult result, SpellCustomErrors customError = SPELL_CUSTOM_ERROR_NONE, uint32* param1 = nullptr,
  ...
  protected:
    ...
    template <class Container> void DoProcessTargetContainer(Container& targetContainer);
    ...
    {
        HitTriggerSpell(SpellInfo const* spellInfo, SpellInfo const* auraSpellInfo, int32 procChance) : triggeredSpell(spellInfo), triggeredByAura(auraSpellInfo), chance(procChance) {}
...
namespace Firelands
{
    struct FC_GAME_API WorldObjectSpellTargetCheck
    {
        Unit* _caster;
        Unit* _referer;
    ...
    struct FC_GAME_API WorldObjectSpellNearbyTargetCheck : public WorldObjectSpellTargetCheck
    {
        float _range;
    ...
    struct FC_GAME_API WorldObjectSpellAreaTargetCheck : public WorldObjectSpellTargetCheck
    {
        float _range;
        Position const* _position;
    ...
    struct FC_GAME_API WorldObjectSpellConeTargetCheck : public WorldObjectSpellAreaTargetCheck
    {
        Position _coneSrc;
        float _coneAngle;
    ...
    struct FC_GAME_API WorldObjectSpellTrajTargetCheck : public WorldObjectSpellTargetCheck
    {
        float _range;
        Position const* _position;
        WorldObjectSpellTrajTargetCheck(float range, Position const* position, Unit* caster, SpellInfo const* spellInfo, SpellTargetCheckTypes selectionType, ConditionContainer* condList);
        bool operator()(WorldObject* target);
    };
} // namespace Firelands
