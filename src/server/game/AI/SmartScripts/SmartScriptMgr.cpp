...
void SmartWaypointMgr::LoadFromDB()
{
    ...
    do
    {
        ...
        if (lastId != id)
            LOG_ERROR("sql.sql", "SmartWaypointMgr::LoadFromDB: Path entry %u, unexpected point id %u, expected %u.", entry, id, lastId);
...
void SmartAIMgr::LoadSmartAIFromDB()
{
    ...
    if (!result)
    {
        LOG_INFO("server.loading", ">> Loaded 0 SmartAI scripts. DB table `smartai_scripts` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (temp.entryOrGuid >= 0)
        {
            ...
            case SMART_SCRIPT_TYPE_CREATURE:
            {
                ...
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry (%u) is not using SmartAI, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
            case SMART_SCRIPT_TYPE_GAMEOBJECT:
            {
                ...
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry (%u) is not using SmartGameObjectAI, skipped loading.", uint32(temp.entryOrGuid));
                    continue;
                }
                break;
            }
        ...
        else
        {
            ...
            case SMART_SCRIPT_TYPE_CREATURE:
            {
                ...
                if (!creatureInfo)
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry (%u) guid (%u) does not exist, skipped loading.", creature->id, uint32(std::abs(temp.entryOrGuid)));
                    continue;
                }

                if (creatureInfo->AIName != "SmartAI")
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Creature entry (%u) guid (%u) is not using SmartAI, skipped loading.", creature->id, uint32(std::abs(temp.entryOrGuid)));
                    continue;
                }
                break;
            }
            case SMART_SCRIPT_TYPE_GAMEOBJECT:
            {
                ...
                if (!gameObject)
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject guid (%u) does not exist, skipped loading.", uint32(std::abs(temp.entryOrGuid)));
                    continue;
                }
                ...
                if (!gameObjectInfo)
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry (%u) guid (%u) does not exist, skipped loading.", gameObject->id, uint32(std::abs(temp.entryOrGuid)));
                    continue;
                }

                if (gameObjectInfo->AIName != "SmartGameObjectAI")
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GameObject entry (%u) guid (%u) is not using SmartGameObjectAI, skipped loading.", gameObject->id,
                        uint32(std::abs(temp.entryOrGuid)));
                    continue;
                }
                break;
            }
            default:
                LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: GUID-specific scripting not yet implemented for source_type %u", (uint32)source_type);
                continue;
            }
        }
        ...
        case SMART_EVENT_TARGET_BUFFED:
            if (temp.event.minMaxRepeat.repeatMin == 0 && temp.event.minMaxRepeat.repeatMax == 0 && !(temp.event.event_flags & SMART_EVENT_FLAG_NOT_REPEATABLE) &&
                temp.source_type != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST)
            {
                temp.event.event_flags |= SMART_EVENT_FLAG_NOT_REPEATABLE;
                LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry %d SourceType %u, Event %u, Missing Repeat flag.", temp.entryOrGuid, temp.GetScriptType(), temp.event_id);
            }
            break;
        case SMART_EVENT_VICTIM_CASTING:
        case SMART_EVENT_IS_BEHIND_TARGET:
            if (temp.event.minMaxRepeat.min == 0 && temp.event.minMaxRepeat.max == 0 && !(temp.event.event_flags & SMART_EVENT_FLAG_NOT_REPEATABLE) &&
                temp.source_type != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST)
            {
                temp.event.event_flags |= SMART_EVENT_FLAG_NOT_REPEATABLE;
                LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry %d SourceType %u, Event %u, Missing Repeat flag.", temp.entryOrGuid, temp.GetScriptType(), temp.event_id);
        ...
        case SMART_EVENT_FRIENDLY_IS_CC:
            if (temp.event.friendlyCC.repeatMin == 0 && temp.event.friendlyCC.repeatMax == 0 && !(temp.event.event_flags & SMART_EVENT_FLAG_NOT_REPEATABLE) &&
                temp.source_type != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST)
            {
                temp.event.event_flags |= SMART_EVENT_FLAG_NOT_REPEATABLE;
                LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry %d SourceType %u, Event %u, Missing Repeat flag.", temp.entryOrGuid, temp.GetScriptType(), temp.event_id);
        ...
        for (SmartAIEventMap::iterator itr = mEventMap[i].begin(); itr != mEventMap[i].end(); ++itr)
        {
            for (SmartScriptHolder const& e : itr->second)
            {
                if (e.link)
                {
                    ...
                    {
                        LOG_ERROR(
                            "sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry %d SourceType %u, Event %u, Link Event %u not found or invalid.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
                    }
                }
                ...
                if (e.GetEventType() == SMART_EVENT_LINK)
                {
                    ...
                    {
                        LOG_ERROR("sql.sql", "SmartAIMgr::LoadSmartAIFromDB: Entry %d SourceType %u, Event %u, Link Source Event not found or invalid. Event will never trigger.", e.entryOrGuid,
...
{
    SmartAIEventList::iterator itr = std::find_if(list.begin(), list.end(), [eventId](SmartScriptHolder& source) { return source.link == eventId; });
...
{
    SmartAIEventList::iterator itr = std::find_if(list.begin(), list.end(), [link](SmartScriptHolder& linked) { return linked.event_id == link && linked.GetEventType() == SMART_EVENT_LINK; });
...
bool SmartAIMgr::IsTargetValid(SmartScriptHolder const& e)
{
    if (std::abs(e.target.o) > 2 * float(M_PI))
        LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has abs(`target.o` = %f) > 2*PI (orientation is expressed in radians)", e.entryOrGuid, e.GetScriptType(),
            e.event_id, e.GetActionType(), e.target.o);
    ...
    case SMART_TARGET_CREATURE_RANGE:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Creature entry %u as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.target.unitDistance.creature);
            return false;
        }
        break;
    }
    ...
    case SMART_TARGET_GAMEOBJECT_RANGE:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent GameObject entry %u as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.target.goDistance.entry);
            return false;
        }
        break;
    }
    ...
    case SMART_TARGET_INVOKER_PARTY:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has invoker target, but event does not provide any invoker!", e.entryOrGuid, e.GetScriptType(),
    ...
    case SMART_TARGET_SUMMONED_CREATURES:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Creature entry %u as target_param1, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.target.invokerSummon.entry);
            return false;
        }
        break;
    }
    ...
    case SMART_TARGET_HOSTILE_RANDOM_AURA:
        break;
    default:
        LOG_ERROR("sql.sql", "SmartAIMgr: Not handled target_type(%u), Entry %d SourceType %u Event %u Action %u, skipped.", e.GetTargetType(), e.entryOrGuid, e.GetScriptType(), e.event_id,
            e.GetActionType());
        return false;
    }
    return true;
...
bool SmartAIMgr::IsMinMaxValid(SmartScriptHolder const& e, uint32 min, uint32 max)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses min/max params wrong (%u/%u), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), min, max);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsCreatureValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Creature entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsQuestValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Quest entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsGameObjectValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent GameObject entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsSpellValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Spell entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsTextEmoteValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Text Emote entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsEmoteValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Emote entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsAreaTriggerValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent AreaTrigger entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
            entry);
        return false;
    }
    return true;
...
bool SmartAIMgr::IsSoundValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Sound entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsAnimKitValid(SmartScriptHolder const& e, uint32 entry)
{
    ...
    {
        LOG_ERROR(
            "sql.sql", "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses non-existent AnimKit entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), entry);
        return false;
    }
    return true;
}
...
bool SmartAIMgr::IsEventValid(SmartScriptHolder& e)
{
    ...
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid %d using event(%u) has invalid action type (%u), skipped.", e.entryOrGuid, e.event_id, e.GetActionType());
        return false;
    }
    ...
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: EntryOrGuid %d SourceType %u, Event %u, Event is linking self (infinite loop), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id);
        return false;
    }
    ...
    else
    {
        ...
        case SMART_EVENT_SPELLHIT_TARGET:
            if (e.event.spellHit.spell)
            {
                ...
                if (!spellInfo)
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Spell entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                ...
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses Spell entry %u with invalid school mask, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                        e.GetActionType(), e.event.spellHit.spell);
                    return false;
                }
            }
        ...
        case SMART_EVENT_RESPAWN:
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Map entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                    e.event.respawn.map);
                return false;
            }
            if (e.event.respawn.type == SMART_SCRIPT_RESPAWN_CONDITION_AREA && !sAreaTableStore.LookupEntry(e.event.respawn.area))
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Area entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                    e.event.respawn.area);
                return false;
            }
            break;
        case SMART_EVENT_FRIENDLY_HEALTH:
        ...
        case SMART_EVENT_MOVEMENTINFORM:
        {
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid Motion type %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                    e.event.movementInform.type);
                return false;
            }
            break;
        }
        ...
        case SMART_EVENT_EVENT_PHASE_CHANGE:
        {
            ...
            {
                LOG_ERROR(
                    "sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has no param set, event won't be executed!.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
                return false;
            }

            if (e.event.eventPhaseChange.phasemask > SMART_EVENT_PHASE_ALL)
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid phasemask %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                    e.event.eventPhaseChange.phasemask);
                return false;
            }

            if (e.event.event_phase_mask && !(e.event.event_phase_mask & e.event.eventPhaseChange.phasemask))
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses event phasemask %u and incompatible event_param1 %u, skipped.", e.entryOrGuid, e.GetScriptType(),
                    e.event_id, e.GetActionType(), e.event.event_phase_mask, e.event.eventPhaseChange.phasemask);
                return false;
            }
            break;
        }
        ...
        case SMART_EVENT_ACTION_DONE:
        {
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid event id %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                    e.event.doAction.eventId);
                return false;
            }
            break;
        }
        ...
        case SMART_EVENT_ON_SPELLCLICK:
            break;
        default:
            LOG_ERROR("sql.sql", "SmartAIMgr: Not handled event_type(%u), Entry %d SourceType %u Event %u Action %u, skipped.", e.GetEventType(), e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType());
            return false;
        }
    }
    ...
    case SMART_ACTION_SET_FACTION:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Faction %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    case SMART_ACTION_MOUNT_TO_ENTRY_OR_MODEL:
        if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
        {
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Creature entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
            ...
            if (e.action.morphOrMount.model)
            {
                ...
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has ModelID set with also set CreatureId, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                        e.GetActionType());
                    return false;
                }
                else if (!sCreatureDisplayInfoStore.LookupEntry(e.action.morphOrMount.model))
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Model id %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
    ...
    case SMART_ACTION_PLAY_ANIMKIT:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses invalid AnimKit type %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.animKit.type);
            return false;
        }
        break;
    ...
    case SMART_ACTION_ACTIVATE_TAXI:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses invalid Taxi path ID %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.taxi.id);
            return false;
        }
        break;
    ...
    case SMART_ACTION_RANDOM_SOUND:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u does not have any non-zero sound", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        ...
        for (uint32 j = 0; j < MAX_SPELL_EFFECTS; ++j)
        {
            if (spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT) || spellInfo->Effects[j].IsEffect(SPELL_EFFECT_KILL_CREDIT2))
            {
                if (spellInfo->Effects[j].TargetA.GetTarget() == TARGET_UNIT_CASTER)
                    LOG_ERROR("sql.sql",
                        "SmartAIMgr: Entry %d SourceType %u Event %u Action %u Effect: SPELL_EFFECT_KILL_CREDIT: (SpellId: %u targetA: %u - targetB: %u) has invalid target for this Action",
    ...
    case SMART_ACTION_INVOKER_CAST:
        if (e.GetScriptType() != SMART_SCRIPT_TYPE_TIMED_ACTIONLIST && e.GetEventType() != SMART_EVENT_LINK && !EventHasInvoker(e.event.type))
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u has invoker cast action, but event does not provide any invoker!", e.entryOrGuid, e.GetScriptType(),
                e.GetEventType(), e.GetActionType());
            return false;
        }
        [[fallthrough]];
    ...
    case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
        if (Quest const* qid = sObjectMgr->GetQuestTemplate(e.action.quest.quest))
        {
            ...
            {
                LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u SpecialFlags for Quest entry %u does not include FLAGS_EXPLORATION_OR_EVENT(2), skipped.", e.entryOrGuid,
                    e.GetScriptType(), e.event_id, e.GetActionType(), e.action.quest.quest);
                return false;
            }
        }
        else
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Quest entry %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    case SMART_ACTION_SET_EVENT_PHASE:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u attempts to set phase %u. Phase mask cannot be used past phase %u, skipped.", e.entryOrGuid, e.GetScriptType(),
                e.event_id, e.GetActionType(), e.action.setEventPhase.phase, SMART_EVENT_PHASE_MAX - 1);
            return false;
        }
        break;
    case SMART_ACTION_INC_EVENT_PHASE:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u attempts to increment phase by too large value, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType());
            return false;
        }
        break;
    ...
    case SMART_ACTION_RANDOM_PHASE:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u does not have any non-zero phase", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
    ...
    {
        if (e.action.randomPhaseRange.phaseMin >= SMART_EVENT_PHASE_MAX || e.action.randomPhaseRange.phaseMax >= SMART_EVENT_PHASE_MAX)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u attempts to set invalid phase, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
    ...
    case SMART_ACTION_SUMMON_CREATURE:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u creature summon: There is a summon spell for creature entry %u (SpellId: %u, effect: %u)", e.entryOrGuid,
                e.GetScriptType(), e.event_id, e.GetActionType(), e.action.summonCreature.creature, itr->second.first, itr->second.second);

        if (e.action.summonCreature.type < TEMPSUMMON_TIMED_OR_DEAD_DESPAWN || e.action.summonCreature.type > TEMPSUMMON_MANUAL_DESPAWN)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses incorrect TempSummonType %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    case SMART_ACTION_CALL_KILLEDMONSTER:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u Kill Credit: There is a killcredit spell for creatureEntry %u (SpellId: %u effect: %u)", e.entryOrGuid,
                e.GetScriptType(), e.event_id, e.GetActionType(), e.action.killedMonster.creature, itr->second.first, itr->second.second);

        if (e.GetTargetType() == SMART_TARGET_POSITION)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses incorrect TargetType %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.GetTargetType());
            return false;
        }
        break;
    ...
    case SMART_ACTION_SET_SHEATH:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses incorrect Sheath state %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    case SMART_ACTION_SUMMON_GO:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u gameobject summon: There is a summon spell for gameobject entry %u (SpellId: %u, effect: %u)", e.entryOrGuid,
                e.GetScriptType(), e.event_id, e.GetActionType(), e.action.summonGO.entry, itr->second.first, itr->second.second);
        break;
    }
    ...
    case SMART_ACTION_ADD_ITEM:
    {
        ...
        for (CacheSpellContainer::const_iterator itr = sBounds.first; itr != sBounds.second; ++itr)
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u Create Item: There is a create item spell for item %u (SpellId: %u effect: %u)", e.entryOrGuid,
                e.GetScriptType(), e.event_id, e.GetActionType(), e.action.item.entry, itr->second.first, itr->second.second);
        break;
    }
    ...
    case SMART_ACTION_INSTALL_AI_TEMPLATE:
        ...
        {
            LOG_ERROR(
                "sql.sql", "SmartAIMgr: Creature %d Event %u Action %u uses non-existent AI template id %u, skipped.", e.entryOrGuid, e.event_id, e.GetActionType(), e.action.installTtemplate.id);
            return false;
        }
        break;
    ...
    case SMART_ACTION_REMOVE_POWER:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent Power %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.power.powerType);
            return false;
        }
        break;
    case SMART_ACTION_GAME_EVENT_STOP:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses non-existent event, eventId %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.gameEventStop.id);
            return false;
        }
    ...
    case SMART_ACTION_EQUIP:
    {
        if (e.GetScriptType() == SMART_SCRIPT_TYPE_CREATURE)
        {
            ...
            if (equipId)
            {
                ...
                if (!einfo)
                {
                    LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id %u for creature %u, skipped.", equipId, e.entryOrGuid);
                    return false;
                }
            }
        }
    ...
    case SMART_ACTION_SET_INGAME_PHASE_ID:
    {
        ...
        if (apply != 0 && apply != 1)
        {
            LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid apply value %u (Should be 0 or 1) for creature %u, skipped", apply, e.entryOrGuid);
            return false;
        }

        if (!sPhaseStore.LookupEntry(phaseId))
        {
            LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_ID uses invalid phaseid %u for creature %u, skipped", phaseId, e.entryOrGuid);
            return false;
        }
        break;
    }
    case SMART_ACTION_SET_INGAME_PHASE_GROUP:
    {
        ...
        if (apply != 0 && apply != 1)
        {
            LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_GROUP uses invalid apply value %u (Should be 0 or 1) for creature %u, skipped", apply, e.entryOrGuid);
            return false;
        }

        if (!sDBCManager.GetPhasesForGroup(phaseGroup))
        {
            LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_SET_INGAME_PHASE_GROUP uses invalid phase group id %u for creature %u, skipped", phaseGroup, e.entryOrGuid);
            return false;
        }
        break;
    }
    case SMART_ACTION_SET_INST_DATA:
    {
        ...
        {
            LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u uses invalid data type %u (value range 0-1), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.setInstanceData.type);
            return false;
        }
        else if (e.action.setInstanceData.type == 1)
        {
            ...
            {
                LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u uses invalid boss state %u (value range 0-5), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                    e.GetActionType(), e.action.setInstanceData.data);
                return false;
            }
        }
        break;
    ...
    case SMART_ACTION_REMOVE_AURAS_BY_TYPE:
    {
        ...
        {
            LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u uses invalid data type %u (value range 0-TOTAL_AURAS), skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.action.auraType.type);
            return false;
        }
        break;
    }
    case SMART_ACTION_RESPAWN_BY_SPAWNID:
        ...
        {
            LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u specifies invalid spawn data (%u,%u)", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
    ...
    case SMART_ACTION_ENABLE_TEMP_GOBJ:
        ...
        {
            LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u does not specify duration", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        break;
    case SMART_ACTION_PAUSE_MOVEMENT:
        ...
        {
            LOG_ERROR("sql.sql", "Entry %u SourceType %u Event %u Action %u does not specify pause duration", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType());
            return false;
        }
        break;
    case SMART_ACTION_SET_MOVEMENT_SPEED:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %u SourceType %u Event %u Action %u uses invalid movementType %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.movementSpeed.movementType);
            return false;
        }
    ...
    case SMART_ACTION_OVERRIDE_LIGHT:
    {
        ...
        if (!areaEntry)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent zoneId %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideLight.zoneId);
            return false;
        }

        if (areaEntry->ParentAreaID != 0)
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses subzone (ID: %u) instead of zone, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.action.overrideLight.zoneId);
            return false;
        }

        if (!sLightStore.LookupEntry(e.action.overrideLight.areaLightId))
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent areaLightId %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideLight.areaLightId);
            return false;
        }

        if (e.action.overrideLight.overrideLightId && !sLightStore.LookupEntry(e.action.overrideLight.overrideLightId))
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent overrideLightId %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.action.overrideLight.overrideLightId);
            return false;
        }
    ...
    case SMART_ACTION_OVERRIDE_WEATHER:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent zoneId %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.overrideWeather.zoneId);
            return false;
        }
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses subzone (ID: %u) instead of zone, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                e.GetActionType(), e.action.overrideWeather.zoneId);
            return false;
        }
    ...
    case SMART_ACTION_SET_AI_ANIM_KIT:
        ...
        {
            LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u uses non-existent AI anim Kit %u, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(),
                e.action.setAIAnimKit.animKitId);
            return false;
        }
        break;
    ...
    case SMART_ACTION_IGNORE_PATHFINDING:
        break;
    default:
        LOG_ERROR("sql.sql", "SmartAIMgr: Not handled action_type(%u), event_type(%u), Entry %d SourceType %u Event %u, skipped.", e.GetActionType(), e.GetEventType(), e.entryOrGuid,
            e.GetScriptType(), e.event_id);
        return false;
    }
...
bool SmartAIMgr::IsTextValid(SmartScriptHolder const& e, uint32 id)
{
    ...
    else
    {
        ...
        default:
            if (e.entryOrGuid < 0)
            {
                ...
                if (!data)
                {
                    LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u using non-existent Creature guid %d, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id,
                        e.GetActionType(), guid);
                    return false;
                }
                else
                    entry = data->id;
    ...
    {
        LOG_ERROR("sql.sql", "SmartAIMgr: Entry %d SourceType %u Event %u Action %u using non-existent Text id %d, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.GetActionType(), id);
        return false;
    }
...
void SmartAIMgr::LoadHelperStores()
{
    ...
    LOG_INFO("server.loading", ">> Loaded SmartAIMgr Helpers in %u ms", GetMSTimeDiffToNow(oldMSTime));
}
...
CacheSpellContainerBounds SmartAIMgr::GetSummonCreatureSpellContainerBounds(uint32 creatureEntry) const { return SummonCreatureSpellStore.equal_range(creatureEntry); }

CacheSpellContainerBounds SmartAIMgr::GetSummonGameObjectSpellContainerBounds(uint32 gameObjectEntry) const { return SummonGameObjectSpellStore.equal_range(gameObjectEntry); }

CacheSpellContainerBounds SmartAIMgr::GetKillCreditSpellContainerBounds(uint32 killCredit) const { return KillCreditSpellStore.equal_range(killCredit); }

CacheSpellContainerBounds SmartAIMgr::GetCreateItemSpellContainerBounds(uint32 itemId) const { return CreateItemSpellStore.equal_range(itemId); }
