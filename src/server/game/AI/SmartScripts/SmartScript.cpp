...
bool SmartScript::IsSmart(Creature* c, bool silent)
{
    ...
    if (!smart && !silent)
        LOG_ERROR("sql.sql", "SmartScript: Action target Creature (GUID: %u Entry: %u) is not using SmartAI, action called by Creature (GUID: %u Entry: %u) skipped to prevent crash.",
            c ? c->GetSpawnId() : 0, c ? c->GetEntry() : 0, me ? me->GetSpawnId() : 0, me ? me->GetEntry() : 0);
...
bool SmartScript::IsSmart(GameObject* g, bool silent)
{
    ...
    if (!smart && !silent)
        LOG_ERROR("sql.sql", "SmartScript: Action target GameObject (GUID: %u Entry: %u) is not using SmartGameObjectAI, action called by GameObject (GUID: %u Entry: %u) skipped to prevent crash.",
            g ? g->GetSpawnId() : 0, g ? g->GetEntry() : 0, go ? go->GetSpawnId() : 0, go ? go->GetEntry() : 0);
...
Creature* SmartScript::FindCreatureNear(WorldObject* searchObject, ObjectGuid::LowType guid) const
{
    ...
    auto creatureItr = std::find_if(bounds.first, bounds.second, [](Map::CreatureBySpawnIdContainer::value_type const& pair) { return pair.second->IsAlive(); });
...
void SmartScript::ProcessAction(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, SpellInfo const* spell, GameObject* gob)
{
    ...
    if (Unit* tempInvoker = GetLastInvoker())
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: Invoker: %s (guidlow: %u)", tempInvoker->GetName().c_str(), tempInvoker->GetGUID().GetCounter());
    ...
    case SMART_ACTION_TALK:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_TALK: talker: %s (GuidLow: %u), textGuid: %u", talker->GetName().c_str(), talker->GetGUID().GetCounter(),
            talkTarget ? talkTarget->GetGUID().GetCounter() : 0);
        break;
    }
        ...
        for (WorldObject* target : targets)
        {
            ...
            else if (IsPlayer(target) && me)
            {
                ...
                sCreatureTextMgr->SendChat(
                    me, uint8(e.action.talk.textGroupID), IsPlayer(templastInvoker) ? templastInvoker : 0, CHAT_MSG_ADDON, LANG_ADDON, TEXT_RANGE_NORMAL, 0, TEAM_OTHER, false, target->ToPlayer());
            }
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SIMPLE_TALK: talker: %s (GuidLow: %u), textGroupId: %u", target->GetName().c_str(), target->GetGUID().GetCounter(),
    ...
    case SMART_ACTION_PLAY_EMOTE:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_PLAY_EMOTE: target: %s (GuidLow: %u), emote: %u", target->GetName().c_str(), target->GetGUID().GetCounter(),
    ...
    case SMART_ACTION_SOUND:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SOUND: target: %s (GuidLow: %u), sound: %u, onlyself: %u", target->GetName().c_str(), target->GetGUID().GetCounter(),
                    e.action.sound.sound, e.action.sound.onlySelf);
            }
        }
        break;
    }
        ...
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                if (e.action.faction.factionID)
                {
                    ...
                    LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry %u, GuidLow %u set faction to %u", target->GetEntry(),
                        target->GetGUID().GetCounter(), e.action.faction.factionID);
                }
                else
                {
                    if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(target->ToCreature()->GetEntry()))
                    {
                        if (target->ToCreature()->GetFaction() != ci->faction)
                        {
                            ...
                            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_FACTION: Creature entry %u, GuidLow %u set faction to %u", target->GetEntry(),
                                target->GetGUID().GetCounter(), ci->faction);
                        }
                    }
                }
            }
            ...
            if (e.action.morphOrMount.creature || e.action.morphOrMount.model)
            {
                ...
                if (e.action.morphOrMount.creature)
                {
                    if (CreatureTemplate const* ci = sObjectMgr->GetCreatureTemplate(e.action.morphOrMount.creature))
                    {
                        ...
                        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u set displayid to %u", target->GetEntry(),
                            target->GetGUID().GetCounter(), displayId);
                    }
                }
                ...
                else
                {
                    ...
                    LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u set displayid to %u", target->GetEntry(),
                        target->GetGUID().GetCounter(), e.action.morphOrMount.model);
                }
            }
            else
            {
                ...
                LOG_DEBUG(
                    "scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_MORPH_TO_ENTRY_OR_MODEL: Creature entry %u, GuidLow %u demorphs.", target->GetEntry(), target->GetGUID().GetCounter());
            }
        }
        break;
    }
    case SMART_ACTION_FAIL_QUEST:
    {
        for (WorldObject* target : targets)
        {
            if (IsPlayer(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_FAIL_QUEST: Player guidLow %u fails quest %u", target->GetGUID().GetCounter(), e.action.quest.quest);
            }
        }
        break;
    }
                ...
                if (Quest const* q = sObjectMgr->GetQuestTemplate(e.action.questOffer.questID))
                {
                    if (me && e.action.questOffer.directAdd == 0)
                    {
                        if (pTarget->CanTakeQuest(q, true))
                        {
                            ...
                            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_OFFER_QUEST: Player guidLow %u - offered quest %u", pTarget->GetGUID().GetCounter(),
                    ...
                    else
                    {
                        ...
                        LOG_DEBUG(
                            "scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_OFFER_QUEST: Player guidLow %u - quest %u added", pTarget->GetGUID().GetCounter(), e.action.questOffer.questID);
                    }
                }
            }
        }
    ...
    case SMART_ACTION_RANDOM_EMOTE:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_EMOTE: Creature guidLow %u handle random emote %u", target->GetGUID().GetCounter(), emote);
            }
        }
        break;
    }
    case SMART_ACTION_THREAT_ALL_PCT:
    {
        ...
        for (auto* ref : me->GetThreatManager().GetModifiableThreatList())
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_THREAT_ALL_PCT: Creature guidLow %u modify threat for unit %u, value %i", me->GetGUID().GetCounter(),
                ref->GetVictim()->GetGUID().GetCounter(), int32(e.action.threatPCT.threatINC) - int32(e.action.threatPCT.threatDEC));
        }
        break;
    }
    case SMART_ACTION_THREAT_SINGLE_PCT:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_THREAT_SINGLE_PCT: Creature guidLow %u modify threat for unit %u, value %i", me->GetGUID().GetCounter(),
                    target->GetGUID().GetCounter(), int32(e.action.threatPCT.threatINC) - int32(e.action.threatPCT.threatDEC));
            }
        }
        break;
    }
    case SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS:
    {
        for (WorldObject* target : targets)
        {
            ...
            if (IsPlayer(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CALL_AREAEXPLOREDOREVENTHAPPENS: Player guidLow %u credited quest %u", target->GetGUID().GetCounter(),
            ...
            if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.cast.spell))
            {
                ...
                if (me)
                {
                    ...
                    if (e.action.cast.castFlags & SMARTCAST_COMBAT_MOVE)
                    {
                        ...
                        if (me->GetDistance(target) > spellInfo->GetMaxRange(true) || me->GetDistance(target) < spellInfo->GetMinRange(true) || !me->IsWithinLOSInMap(target) ||
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_CAST:: %s: %u casts spell %u on target %u with castflags %u", (me ? me->GetGUID() : go->GetGUID()).GetTypeName(),
                    me ? me->GetGUID().GetCounter() : go->GetGUID().GetCounter(), e.action.cast.spell, target->GetGUID().GetCounter(), e.action.cast.castFlags);
            }
            else
                LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", e.action.cast.spell,
    ...
    case SMART_ACTION_INVOKER_CAST:
    {
        ...
        for (WorldObject* target : targets)
        {
            ...
            if (!(e.action.cast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.cast.spell))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_INVOKER_CAST: Invoker %u casts spell %u on target %u with castflags %u", tempLastInvoker->GetGUID().GetCounter(),
                    e.action.cast.spell, target->GetGUID().GetCounter(), e.action.cast.castFlags);
            }
            else
                LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", e.action.cast.spell,
                    target->GetGUID().ToString().c_str());
    ...
    case SMART_ACTION_ADD_AURA:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ADD_AURA: Adding aura %u to unit %u", e.action.cast.spell, target->GetGUID().GetCounter());
            }
        }
        break;
    }
    case SMART_ACTION_ACTIVATE_GOBJECT:
    {
        for (WorldObject* target : targets)
        {
            if (IsGameObject(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ACTIVATE_GOBJECT. Gameobject %u (entry: %u) activated", target->GetGUID().GetCounter(), target->GetEntry());
            }
        }
        break;
    }
    case SMART_ACTION_RESET_GOBJECT:
    {
        for (WorldObject* target : targets)
        {
            if (IsGameObject(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RESET_GOBJECT. Gameobject %u (entry: %u) reset", target->GetGUID().GetCounter(), target->GetEntry());
            }
        }
        break;
    }
    case SMART_ACTION_SET_EMOTE_STATE:
    {
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_EMOTE_STATE. Unit %u set emotestate to %u", target->GetGUID().GetCounter(), e.action.emote.emote);
            }
        }
        break;
    }
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                if (!e.action.unitFlag.type)
                {
                    ...
                    LOG_DEBUG(
                        "scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS", target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
                else
                {
                    target->ToUnit()->SetFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                    LOG_DEBUG(
                        "scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_UNIT_FLAG. Unit %u added flag %u to UNIT_FIELD_FLAGS_2", target->GetGUID().GetCounter(), e.action.unitFlag.flag);
                }
            }
        }
        break;
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                if (!e.action.unitFlag.type)
                {
                    ...
                    LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS", target->GetGUID().GetCounter(),
                        e.action.unitFlag.flag);
                }
                else
                {
                    target->ToUnit()->RemoveFlag(UNIT_FIELD_FLAGS_2, e.action.unitFlag.flag);
                    LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_REMOVE_UNIT_FLAG. Unit %u removed flag %u to UNIT_FIELD_FLAGS_2", target->GetGUID().GetCounter(),
                        e.action.unitFlag.flag);
                }
            }
        }
    ...
    case SMART_ACTION_AUTO_ATTACK:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_AUTO_ATTACK: Creature: %u bool on = %u", me->GetGUID().GetCounter(), e.action.autoAttack.attack);
        break;
    }
    case SMART_ACTION_ALLOW_COMBAT_MOVEMENT:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_ALLOW_COMBAT_MOVEMENT: Creature %u bool on = %u", me->GetGUID().GetCounter(), e.action.combatMove.move);
        break;
    }
    case SMART_ACTION_SET_EVENT_PHASE:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SET_EVENT_PHASE: Creature %u set event phase %u", GetBaseObject()->GetGUID().GetCounter(), e.action.setEventPhase.phase);
        break;
    }
    case SMART_ACTION_INC_EVENT_PHASE:
    {
        ...
        LOG_DEBUG("scripts.ai",
            "SmartScript::ProcessAction:: SMART_ACTION_INC_EVENT_PHASE: Creature %u inc event phase by %u, "
            "decrease by %u",
            GetBaseObject()->GetGUID().GetCounter(), e.action.incEventPhase.inc, e.action.incEventPhase.dec);
        break;
    }
    case SMART_ACTION_EVADE:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_EVADE: Creature %u EnterEvadeMode", me->GetGUID().GetCounter());
        break;
    }
    case SMART_ACTION_FLEE_FOR_ASSIST:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_FLEE_FOR_ASSIST: Creature %u DoFleeToGetAssistance", me->GetGUID().GetCounter());
        break;
    }
    case SMART_ACTION_CALL_GROUPEVENTHAPPENS:
    {
        ...
        if (player && GetBaseObject())
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_GROUPEVENTHAPPENS: Player %u, group credit for quest %u", unit->GetGUID().GetCounter(), e.action.quest.quest);
        }
    ...
    case SMART_ACTION_COMBAT_STOP:
    {
        ...
        me->CombatStop(true);
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_COMBAT_STOP: %s CombatStop", me->GetGUID().ToString().c_str());
        break;
    }
    case SMART_ACTION_REMOVEAURASFROMSPELL:
    {
        for (WorldObject* target : targets)
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_REMOVEAURASFROMSPELL: Unit %u, spell %u", target->GetGUID().GetCounter(), e.action.removeAura.spell);
        }
        break;
    }
    case SMART_ACTION_FOLLOW:
    {
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_FOLLOW: Creature %u following target %u", me->GetGUID().GetCounter(), target->GetGUID().GetCounter());
                break;
            }
        }
        break;
    ...
    case SMART_ACTION_RANDOM_PHASE:
    {
        ...
        std::copy_if(std::begin(e.action.randomPhase.phases), std::end(e.action.randomPhase.phases), std::back_inserter(phases), [](uint32 phase) { return phase != 0; });

        uint32 phase = Firelands::Containers::SelectRandomContainerElement(phases);
        SetPhase(phase);
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE: Creature %u sets event phase to %u", GetBaseObject()->GetGUID().GetCounter(), phase);
        break;
    }
    case SMART_ACTION_RANDOM_PHASE_RANGE:
    {
        ...
        SetPhase(phase);
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_RANDOM_PHASE_RANGE: Creature %u sets event phase to %u", GetBaseObject()->GetGUID().GetCounter(), phase);
        break;
    }
    case SMART_ACTION_CALL_KILLEDMONSTER:
    {
        if (e.target.type == SMART_TARGET_NONE || e.target.type == SMART_TARGET_SELF) // Loot recipient and his group members
        {
            ...
            if (Player* player = me->GetLootRecipient())
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u", player->GetGUID().GetCounter(), e.action.killedMonster.creature);
            }
        }
        else // Specific target type
        {
            for (WorldObject* target : targets)
            {
                if (IsPlayer(target))
                {
                    ...
                    LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_KILLEDMONSTER: Player %u, Killcredit: %u", target->GetGUID().GetCounter(), e.action.killedMonster.creature);
                }
    ...
    case SMART_ACTION_SET_INST_DATA:
    {
        ...
        case 0:
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: SetData Field: %u, data: %u", e.action.setInstanceData.field, e.action.setInstanceData.data);
            break;
        case 1:
            instance->SetBossState(e.action.setInstanceData.field, static_cast<EncounterState>(e.action.setInstanceData.data));
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA: SetBossState BossId: %u, State: %u (%s)", e.action.setInstanceData.field, e.action.setInstanceData.data,
                InstanceScript::GetBossStateName(e.action.setInstanceData.data));
            break;
    ...
    case SMART_ACTION_SET_INST_DATA64:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_INST_DATA64: Field: %u, data: %s", e.action.setInstanceData64.field, targets.front()->GetGUID().ToString().c_str());
        break;
    }
    ...
    case SMART_ACTION_DIE:
    {
        ...
        {
            me->KillSelf();
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_DIE: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_SET_IN_COMBAT_WITH_ZONE:
    {
        if (me && me->IsAIEnabled)
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_IN_COMBAT_WITH_ZONE: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_CALL_FOR_HELP:
    {
        if (me)
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_CALL_FOR_HELP: Creature %u", me->GetGUID().GetCounter());
        }
        break;
    }
    case SMART_ACTION_SET_SHEATH:
    {
        if (me)
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_SET_SHEATH: Creature %u, State: %u", me->GetGUID().GetCounter(), e.action.setSheath.sheath);
        }
        break;
    }
    ...
    case SMART_ACTION_SUMMON_CREATURE:
    {
        ...
        if (e.GetTargetType() == SMART_TARGET_RANDOM_POINT)
        {
            ...
            for (uint32 i = 0; i < e.target.randomPoint.amount; i++)
            {
                ...
                if (Creature* summon = summoner->SummonCreature(
                        e.action.summonCreature.creature, randomPoint, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, 0, nullptr, personalSpawn))
                {
        ...
        for (WorldObject* target : targets)
        {
            ...
            o += e.target.o;
            if (Creature* summon =
                    summoner->SummonCreature(e.action.summonCreature.creature, x, y, z, o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration, nullptr, personalSpawn))
            {
        ...
        if (Creature* summon = summoner->SummonCreature(
                e.action.summonCreature.creature, e.target.x, e.target.y, e.target.z, e.target.o, (TempSummonType)e.action.summonCreature.type, e.action.summonCreature.duration))
        {
            if (unit && e.action.summonCreature.attackInvoker)
                summon->AI()->AttackStart(unit);
            else if (me && e.action.summonCreature.attackScriptOwner)
    ...
    case SMART_ACTION_SUMMON_GO:
    {
        ...
        GetBaseObject()->SummonGameObject(
            e.action.summonGO.entry, Position(e.target.x, e.target.y, e.target.z, e.target.o), rot, e.action.summonGO.despawnTime, GOSummonType(e.action.summonGO.summonType));
        break;
    }
    ...
    case SMART_ACTION_MOVE_TO_POS:
    {
        ...
        case SMART_TARGET_POSITION:
        {
            ...
            me->GetMotionMaster()->MovePoint(e.action.moveToPos.pointId, dest.x, dest.y, dest.z, true, true, isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE, e.target.o);
        ...
        case SMART_TARGET_RANDOM_POINT:
        {
            if (me)
            {
                ...
                me->GetMotionMaster()->MovePoint(
                    e.action.moveToPos.pointId, randomPoint.m_positionX, randomPoint.m_positionY, randomPoint.m_positionZ, true, true, isControlled ? MOTION_SLOT_CONTROLLED : MOTION_SLOT_ACTIVE);
            }
    ...
    case SMART_ACTION_ENABLE_TEMP_GOBJ:
    {
        ...
        {
            if (IsCreature(target))
                LOG_WARN("sql.sql", "Invalid creature target '%s' (entry %u, spawnId %u) specified for SMART_ACTION_ENABLE_TEMP_GOBJ", target->GetName().c_str(), target->GetEntry(),
                    target->ToCreature()->GetSpawnId());
            else if (IsGameObject(target))
            {
                if (target->ToGameObject()->isSpawnedByDefault())
                    LOG_WARN("sql.sql", "Invalid gameobject target '%s' (entry %u, spawnId %u) for SMART_ACTION_ENABLE_TEMP_GOBJ - the object is spawned by default", target->GetName().c_str(),
                        target->GetEntry(), target->ToGameObject()->GetSpawnId());
                else
        ...
        for (WorldObject* target : targets)
        {
            if (Creature* npc = target->ToCreature())
            {
                ...
                if (equipId)
                {
                    ...
                    if (!einfo)
                    {
                        LOG_ERROR("sql.sql", "SmartScript: SMART_ACTION_EQUIP uses non-existent equipment info id %u for creature %u", equipId, npc->GetEntry());
                        break;
                    }
    ...
    case SMART_ACTION_CALL_TIMED_ACTIONLIST:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartScript: Entry %d SourceType %u Event %u Action %u is using TARGET_NONE(0) for Script9 target. Please correct target_type in database.", e.entryOrGuid,
                e.GetScriptType(), e.GetEventType(), e.GetActionType());
            break;
        }
    ...
    case SMART_ACTION_CROSS_CAST:
    {
        ...
        GetTargets(casters,
            CreateSmartEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, 0, 0, 0, SMART_ACTION_NONE, 0, 0, 0, 0, 0, 0, (SMARTAI_TARGETS)e.action.crossCast.targetType, e.action.crossCast.targetParam1,
                e.action.crossCast.targetParam2, e.action.crossCast.targetParam3, 0),
            unit);
        ...
        for (WorldObject* caster : casters)
        {
            ...
            for (WorldObject* target : targets)
            {
                ...
                if (!(e.action.crossCast.castFlags & SMARTCAST_AURA_NOT_PRESENT) || !target->ToUnit()->HasAura(e.action.crossCast.spell))
                {
                    ...
                    LOG_DEBUG("scripts.ai", "Spell %u not cast because it has flag SMARTCAST_AURA_NOT_PRESENT and the target (%s) already has the aura", e.action.crossCast.spell,
                        target->GetGUID().ToString().c_str());
            }
        }
        break;
    ...
    case SMART_ACTION_CALL_RANDOM_TIMED_ACTIONLIST:
    {
        ...
        std::copy_if(std::begin(e.action.randTimedActionList.actionLists), std::end(e.action.randTimedActionList.actionLists), std::back_inserter(actionLists),
            ...
            if (WorldObject* target = Acore::Containers::SelectRandomContainerElement(targets))
                if (me)
                    me->GetMotionMaster()->MoveJump(
                        target->GetPositionX() + e.target.x, target->GetPositionY() + e.target.y, target->GetPositionZ() + e.target.z, (float)e.action.jump.speedxy, (float)e.action.jump.speedz);
        }
        else
        {
            for (WorldObject* target : targets)
    ...
    case SMART_ACTION_SEND_GOSSIP_MENU:
    {
        ...
        LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_SEND_GOSSIP_MENU: gossipMenuId %d, gossipNpcTextId %d", e.action.sendGossipMenu.gossipMenuId,
    ...
    case SMART_ACTION_SET_HOME_POS:
    {
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                ...
                else if (e.GetTargetType() == SMART_TARGET_CREATURE_RANGE || e.GetTargetType() == SMART_TARGET_CREATURE_GUID || e.GetTargetType() == SMART_TARGET_CREATURE_DISTANCE ||
                         e.GetTargetType() == SMART_TARGET_GAMEOBJECT_RANGE || e.GetTargetType() == SMART_TARGET_GAMEOBJECT_GUID || e.GetTargetType() == SMART_TARGET_GAMEOBJECT_DISTANCE ||
                         e.GetTargetType() == SMART_TARGET_CLOSEST_CREATURE || e.GetTargetType() == SMART_TARGET_CLOSEST_GAMEOBJECT || e.GetTargetType() == SMART_TARGET_OWNER_OR_SUMMONER ||
                         e.GetTargetType() == SMART_TARGET_ACTION_INVOKER || e.GetTargetType() == SMART_TARGET_CLOSEST_ENEMY || e.GetTargetType() == SMART_TARGET_CLOSEST_FRIENDLY)
                {
                    target->ToCreature()->SetHomePosition(target->GetPositionX(), target->GetPositionY(), target->GetPositionZ(), target->GetOrientation());
                }
                else
                    LOG_ERROR("sql.sql", "SmartScript: Action target for SMART_ACTION_SET_HOME_POS is invalid, skipping");
            }
        }
    ...
    case SMART_ACTION_GAME_EVENT_STOP:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_STOP, inactive event (id: %u)", eventId);
            break;
        }
    ...
    case SMART_ACTION_GAME_EVENT_START:
    {
        ...
        if (sGameEventMgr->IsActiveEvent(eventId))
        {
            LOG_ERROR("sql.sql", "SmartScript::ProcessAction: At case SMART_ACTION_GAME_EVENT_START, already activated event (id: %u)", eventId);
            break;
        }
    ...
    case SMART_ACTION_START_CLOSEST_WAYPOINT:
    {
        ...
        std::copy_if(std::begin(e.action.closestWaypointFromList.wps), std::end(e.action.closestWaypointFromList.wps), std::back_inserter(waypoints), [](uint32 wp) { return wp != 0; });
    ...
    case SMART_ACTION_RANDOM_SOUND:
    {
        ...
        std::copy_if(std::begin(e.action.randomSound.sounds), std::end(e.action.randomSound.sounds), std::back_inserter(sounds), [](uint32 sound) { return sound != 0; });
        ...
        for (WorldObject* target : targets)
        {
            if (IsUnit(target))
            {
                ...
                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_RANDOM_SOUND: target: %s (%s), sound: %u, onlyself: %s", target->GetName().c_str(),
                    target->GetGUID().ToString().c_str(), sound, onlySelf ? "true" : "false");
            }
        }
        break;
    }
    ...
    case SMART_ACTION_PLAY_ANIMKIT:
    {
        for (WorldObject* target : targets)
        {
            if (IsCreature(target))
            {
                ...
                {
                    LOG_ERROR("sql.sql", "SmartScript: Invalid type for SMART_ACTION_PLAY_ANIMKIT, skipping");
                    break;
                }

                LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction:: SMART_ACTION_PLAY_ANIMKIT: target: %s (%s), AnimKit: %u, Type: %u", target->GetName().c_str(),
                    target->GetGUID().ToString().c_str(), e.action.animKit.animKit, e.action.animKit.type);
            }
        }
        break;
    }
    ...
    case SMART_ACTION_RESPAWN_BY_SPAWNID:
    {
        ...
        if (map)
            ...
            LOG_ERROR("sql.sql", "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u - tries to respawn by spawnId but does not provide a map", e.entryOrGuid, e.GetScriptType(), e.event_id);
        break;
    }
    ...
    case SMART_ACTION_OVERRIDE_LIGHT:
    {
        if (WorldObject* obj = GetBaseObject())
        {
            obj->GetMap()->SetZoneOverrideLight(
                e.action.overrideLight.zoneId, e.action.overrideLight.areaLightId, e.action.overrideLight.overrideLightId, e.action.overrideLight.transitionMilliseconds);
            LOG_DEBUG("scripts.ai",
                "SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_LIGHT: %s sets zone override light (zoneId: %u, areaLightId: %u, overrideLightId: %u, transitionMilliseconds: %u)",
                obj->GetGUID().ToString().c_str(), e.action.overrideLight.zoneId, e.action.overrideLight.areaLightId, e.action.overrideLight.overrideLightId,
                e.action.overrideLight.transitionMilliseconds);
        }
        break;
    }
    case SMART_ACTION_OVERRIDE_WEATHER:
    {
        if (WorldObject* obj = GetBaseObject())
        {
            ...
            LOG_DEBUG("scripts.ai", "SmartScript::ProcessAction: SMART_ACTION_OVERRIDE_WEATHER: %s sets zone weather (zoneId: %u, weatherId: %u, intensity: %u)", obj->GetGUID().ToString().c_str(),
                e.action.overrideWeather.zoneId, e.action.overrideWeather.weatherId, e.action.overrideWeather.intensity);
        }
        break;
    }
    ...
    case SMART_ACTION_UPDATE_ACHIEVEMENT_CRITERIA:
    {
        ...
        for (WorldObject* const target : targets)
        {
            ...
            if (Player* player = target->ToUnit()->ToPlayer())
                player->UpdateAchievementCriteria(static_cast<AchievementCriteriaTypes>(e.action.achievementCriteria.type), e.action.achievementCriteria.misc1, e.action.achievementCriteria.misc2,
                    e.action.achievementCriteria.misc3, unit);
        }
    ...
    if (e.link && e.link != e.event_id)
    {
        ...
        if (linked)
            ...
            LOG_DEBUG("sql.sql", "SmartScript::ProcessAction: Entry %d SourceType %u, Event %u, Link Event %u not found or invalid, skipped.", e.entryOrGuid, e.GetScriptType(), e.event_id, e.link);
    }
}
...
void SmartScript::InstallTemplate(SmartScriptHolder const& e)
{
    ...
    {
        AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2, e.action.installTtemplate.param3, 0, SMART_ACTION_CAST, e.action.installTtemplate.param1, e.target.raw.param1, 0, 0,
            0, 0, SMART_TARGET_VICTIM, 0, 0, 0, 1);
        ...
        AddEvent(SMART_EVENT_RANGE, 0, 0, e.action.installTtemplate.param4 > 10 ? e.action.installTtemplate.param4 - 10 : 0, 0, 0, 0, SMART_ACTION_ALLOW_COMBAT_MOVEMENT, 0, 0, 0, 0, 0, 0,
            SMART_TARGET_NONE, 0, 0, 0, 1);
        AddEvent(SMART_EVENT_MANA_PCT, 0, e.action.installTtemplate.param5 - 15 > 100 ? 100 : e.action.installTtemplate.param5 + 15, 100, 1000, 1000, 0, SMART_ACTION_SET_EVENT_PHASE, 1, 0, 0, 0, 0, 0,
    ...
    {
        AddEvent(SMART_EVENT_UPDATE_IC, 0, 0, 0, e.action.installTtemplate.param2, e.action.installTtemplate.param3, 0, SMART_ACTION_CAST, e.action.installTtemplate.param1, e.target.raw.param1, 0, 0,
            0, 0, SMART_TARGET_VICTIM, 0, 0, 0, 0);
    ...
    case SMARTAI_TEMPLATE_CAGED_NPC_PART:
    {
        ...
        AddEvent(SMART_EVENT_UPDATE, SMART_EVENT_FLAG_NOT_REPEATABLE, 0, 0, 0, 0, 0, SMART_ACTION_RESET_GOBJECT, 0, 0, 0, 0, 0, 0, SMART_TARGET_GAMEOBJECT_DISTANCE, e.action.installTtemplate.param1,
            5, 0, 0);
        ...
        AddEvent(
            SMART_EVENT_MOVEMENTINFORM, 0, POINT_MOTION_TYPE, SMART_RANDOM_POINT, 0, 0, 0, SMART_ACTION_FORCE_DESPAWN, e.action.installTtemplate.param2, 0, 0, 0, 0, 0, SMART_TARGET_NONE, 0, 0, 0, 1);
    ...
    case SMARTAI_TEMPLATE_CAGED_GO_PART:
    {
        ...
        AddEvent(SMART_EVENT_GO_LOOT_STATE_CHANGED, 0, GO_ACTIVATED, 0, 0, 0, 0, SMART_ACTION_STORE_TARGET_LIST, 1, 0, 0, 0, 0, 0, SMART_TARGET_CLOSEST_CREATURE, e.action.installTtemplate.param1, 10,
            0, 0);
...
void SmartScript::AddEvent(SMART_EVENT e, uint32 event_flags, uint32 event_param1, uint32 event_param2, uint32 event_param3, uint32 event_param4, uint32 event_param5, SMART_ACTION action,
    uint32 action_param1, uint32 action_param2, uint32 action_param3, uint32 action_param4, uint32 action_param5, uint32 action_param6, SMARTAI_TARGETS t, uint32 target_param1, uint32 target_param2,
    uint32 target_param3, uint32 phaseMask)
{
    mInstallEvents.push_back(CreateSmartEvent(e, event_flags, event_param1, event_param2, event_param3, event_param4, event_param5, action, action_param1, action_param2, action_param3, action_param4,
        action_param5, action_param6, t, target_param1, target_param2, target_param3, phaseMask));
}

SmartScriptHolder SmartScript::CreateSmartEvent(SMART_EVENT e, uint32 event_flags, uint32 event_param1, uint32 event_param2, uint32 event_param3, uint32 event_param4, uint32 event_param5,
    SMART_ACTION action, uint32 action_param1, uint32 action_param2, uint32 action_param3, uint32 action_param4, uint32 action_param5, uint32 action_param6, SMARTAI_TARGETS t, uint32 target_param1,
    uint32 target_param2, uint32 target_param3, uint32 phaseMask)
{
...
void SmartScript::GetTargets(ObjectVector& targets, SmartScriptHolder const& e, Unit* invoker /*= nullptr*/)
{
    ...
    case SMART_TARGET_HOSTILE_SECOND_AGGRO:
        if (me)
        {
            ...
            {
                if (Unit* u = me->AI()->SelectTarget(
                        SELECT_TARGET_MAXTHREAT, 1, PowerUsersSelector(me, Powers(e.target.hostilRandom.powerType - 1), (float)e.target.hostilRandom.maxDist, e.target.hostilRandom.playerOnly != 0)))
                    targets.push_back(u);
            }
    ...
    case SMART_TARGET_HOSTILE_LAST_AGGRO:
        if (me)
        {
            ...
            {
                if (Unit* u = me->AI()->SelectTarget(
                        SELECT_TARGET_MINTHREAT, 0, PowerUsersSelector(me, Powers(e.target.hostilRandom.powerType - 1), (float)e.target.hostilRandom.maxDist, e.target.hostilRandom.playerOnly != 0)))
                    targets.push_back(u);
            }
    ...
    case SMART_TARGET_HOSTILE_RANDOM:
        if (me)
        {
            ...
            {
                if (Unit* u = me->AI()->SelectTarget(
                        SELECT_TARGET_RANDOM, 0, PowerUsersSelector(me, Powers(e.target.hostilRandom.powerType - 1), (float)e.target.hostilRandom.maxDist, e.target.hostilRandom.playerOnly != 0)))
                    targets.push_back(u);
            }
    ...
    case SMART_TARGET_HOSTILE_RANDOM_NOT_TOP:
        if (me)
        {
            ...
            {
                if (Unit* u = me->AI()->SelectTarget(
                        SELECT_TARGET_RANDOM, 1, PowerUsersSelector(me, Powers(e.target.hostilRandom.powerType - 1), (float)e.target.hostilRandom.maxDist, e.target.hostilRandom.playerOnly != 0)))
                    targets.push_back(u);
            }
    ...
    case SMART_TARGET_FARTHEST:
        ...
        {
            if (Unit* u = me->AI()->SelectTarget(
                    SELECT_TARGET_MAXDISTANCE, 0, FarthestTargetSelector(me, (float)e.target.farthest.maxDist, e.target.farthest.playerOnly != 0, e.target.farthest.isInLos != 0)))
                targets.push_back(u);
        }
        break;
    ...
    case SMART_TARGET_CREATURE_RANGE:
    {
        ...
        for (WorldObject* unit : units)
        {
            ...
            if ((!e.target.unitRange.creature || unit->ToCreature()->GetEntry() == e.target.unitRange.creature) &&
                baseObject->IsInRange(unit, float(e.target.unitRange.minDist), float(e.target.unitRange.maxDist)))
    ...
    case SMART_TARGET_GAMEOBJECT_RANGE:
    {
        ...
        for (WorldObject* unit : units)
        {
            ...
            if ((!e.target.goRange.entry && unit->ToGameObject()->GetEntry() == e.target.goRange.entry) &&
                baseObject->IsInRange(unit, float(e.target.goRange.minDist), float(e.target.goRange.maxDist)))
                targets.push_back(unit);
        }
        break;
    }
    case SMART_TARGET_CREATURE_GUID:
    {
        ...
        {
            LOG_ERROR("sql.sql", "SMART_TARGET_CREATURE_GUID can not be used without invoker");
            break;
        }
    ...
    case SMART_TARGET_GAMEOBJECT_GUID:
    {
        if (!scriptTrigger && !baseObject)
        {
            LOG_ERROR("sql.sql", "SMART_TARGET_GAMEOBJECT_GUID can not be used without invoker");
            break;
        }
    ...
    case SMART_TARGET_CLOSEST_CREATURE:
    {
        ...
        if (!ref)
        {
            LOG_ERROR("scripts.ai.sai", "SMART_TARGET_CLOSEST_CREATURE: Entry %i SourceType %u Event %u Action %u Target %u is missing base object or invoker.", e.entryOrGuid, e.GetScriptType(),
                e.event_id, e.GetActionType(), e.GetTargetType());
            break;
        }
    ...
    case SMART_TARGET_INSTANCE_STORAGE:
    {
        ...
        {
            LOG_ERROR("scripts.ai.sai", "SMART_TARGET_INSTANCE_STORAGE: Entry {} SourceType {} Event {} Action {} Target {} called outside an instance map.", e.entryOrGuid, e.GetScriptType(),
                e.event_id, e.GetActionType(), e.GetTargetType());
        }
...
void SmartScript::ProcessEvent(SmartScriptHolder& e, Unit* unit, uint32 var0, uint32 var1, bool bvar, SpellInfo const* spell, GameObject* gob)
{
    ...
    case SMART_EVENT_SPELLHIT:
    {
        if (!spell)
            return;
        if ((!e.event.spellHit.spell || spell->Id == e.event.spellHit.spell) && (!e.event.spellHit.school || (spell->SchoolMask & e.event.spellHit.school)))
        {
    ...
    case SMART_EVENT_OOC_LOS:
    {
        ...
        if (me->IsWithinDistInMap(unit, range) && me->IsWithinLOSInMap(unit))
        {
            ...
            if ((e.event.los.noHostile && !me->IsHostileTo(unit)) || (!e.event.los.noHostile && me->IsHostileTo(unit)))
            {
    ...
    {
        LOG_DEBUG("scripts.ai", "SmartScript: Gossip Select:  menu %u action %u", var0, var1); // little help for scripters
...
bool SmartScript::CheckTimer(SmartScriptHolder const& e) const { return e.active; }
...
bool SmartScript::IsUnit(WorldObject* obj) { return obj && (obj->GetTypeId() == TYPEID_UNIT || obj->IsPlayer()); }

bool SmartScript::IsPlayer(WorldObject* obj) { return obj && obj->IsPlayer(); }

bool SmartScript::IsCreature(WorldObject* obj) { return obj && obj->GetTypeId() == TYPEID_UNIT; }
...
bool SmartScript::IsGameObject(WorldObject* obj) { return obj && obj->GetTypeId() == TYPEID_GAMEOBJECT; }
...
void SmartScript::OnInitialize(WorldObject* obj, AreaTriggerEntry const* at, Quest const* qst)
{
    if (obj) // handle object based scripts
    {
        ...
        case TYPEID_GAMEOBJECT:
            ...
            break;
        default:
            LOG_ERROR("misc", "SmartScript::OnInitialize: Unhandled TypeID !WARNING!");
            return;
        }
    }
    else if (at)
    ...
    {
        LOG_ERROR("misc", "SmartScript::OnInitialize: !WARNING! Initialized objects are nullptr.");
        return;
    }
...
void SmartScript::SetScript9(SmartScriptHolder& e, uint32 entry)
{
    ...
    {
        LOG_ERROR("scripts.ai", "Entry %d SourceType %u Event %u Action %u is trying to overwrite timed action list from a timed action, this is not allowed!.", e.entryOrGuid, e.GetScriptType(),
            e.GetEventType(), e.GetActionType());
        return;
    }
...
void SmartScript::AddCreatureSummon(ObjectGuid const& guid) { _summonList.insert(guid); }

Creature* SmartScript::GetCreatureSummon(uint32 entry)
{
...
void SmartScript::RemoveCreatureSummon(ObjectGuid const& guid) { _summonList.erase(guid); }
