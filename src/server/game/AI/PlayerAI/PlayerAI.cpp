...
PlayerAI::PlayerAI(Player* player)
    : UnitAI(player), me(player), _selfSpec(PlayerAI::GetPlayerSpec(player)), _isSelfHealer(PlayerAI::IsPlayerHealer(player)),
...
struct ValidTargetSelectPredicate
{
    ...
    bool operator()(Unit const* target) const { return _ai->CanAIAttack(target); }
};
...
PlayerAI::TargetedSpell SimpleCharmedPlayerAI::SelectAppropriateCastForSpec()
{
    ...
    case CLASS_HUNTER:
        ...
        if (me->GetVictim() && me->GetVictim()->GetPowerType() == POWER_MANA &&
            !me->GetVictim()->GetAuraApplicationOfRankedSpell(SPELL_VIPER_STING, me->GetGUID()))
    ...
    case CLASS_DEATH_KNIGHT:
    {
        ...
        if (Unit* victim = me->GetVictim())
        {
            ...
            hasFF = !!victim->GetAuraApplicationOfRankedSpell(AURA_FROST_FEVER, me->GetGUID()),
            hasBP = !!victim->GetAuraApplicationOfRankedSpell(AURA_BLOOD_PLAGUE, me->GetGUID());
            if (hasFF && hasBP)
