    ...
    switch (i)
    {
        case SECRET_TOTP_MASTER_KEY:
        {
            ...
            if (result) do
            {
                ...
                {
                    if (!oldSecret)
                        return Firelands::StringFormat("Cannot decrypt old TOTP tokens - add config key '{}' to authserver.conf!", secret_info[i].oldKey);
                    ...
                    if (!success)
                        return Firelands::StringFormat("Cannot decrypt old TOTP tokens - value of '{}' is incorrect for some users!", secret_info[i].oldKey);
                }
